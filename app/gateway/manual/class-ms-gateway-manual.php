...
class MS_Gateway_Manual extends MS_Gateway {
	...
	public function after_load() {
		...
		$this->id             = self::ID;
		$this->name           = __( 'Manual Payment Gateway', 'memberdash' );
		$this->description    = __( 'Bank orders, cash, etc', 'memberdash' );
		$this->group          = __( 'Manual Payment', 'memberdash' );
	 ...
	 *
	 * Returns a default message if gateway is not configured.
	 *
	...
	public function purchase_info_content() {
		...
		if ( ! $Processed ) {
			...
			if ( empty( $this->payment_info ) ) {
				...
				ob_start();
				?>
					<?php esc_html_e( 'This is only an example of manual payment gateway instructions', 'memberdash' ); ?>
					<br />
					<?php
					printf(
						esc_html__( 'Edit it %1$shere%2$s', 'memberdash' ), // cspell:disable-line.
						'<a href="' . esc_url( $link ) . '">',
						'</a>'
					);
					?>
					<br /><br />
					<?php esc_html_e( 'Name: Example name.', 'memberdash' ); ?>
					<br />
					<?php esc_html_e( 'Bank: Example bank.', 'memberdash' ); ?>
					<br />
					<?php esc_html_e( 'Bank account: Example bank account 1234.', 'memberdash' ); ?>
					<br />
				<?php
