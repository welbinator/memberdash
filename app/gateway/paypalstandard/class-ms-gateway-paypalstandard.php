...
class MS_Gateway_Paypalstandard extends MS_Gateway {
	...
	/**
	 * Paypal IPN listening URL.
	 *
	 * @since 1.0.0
	 * @var string $ipn_listening_url
	 */
	protected $ipn_listening_url;
	...
	public function after_load() {
		...
		$this->id             = self::ID;
		$this->name           = __( 'PayPal Standard Gateway', 'memberdash' );
		$this->group          = 'PayPal';
		...
		if ( $this->active && $this->is_live_mode() && strpos( $this->merchant_id, '@' ) ) {
			...
			mslib3()->ui->admin_message(
				sprintf(
					__( 'Warning: You use your email address for the PayPal Standard gateway instead of your Merchant ID. Please check %1$syour payment settings%2$s and enter the Merchant ID instead', 'memberdash' ), // cspell:disable-line.
					'<a href="' . $settings_url . '">',
					'</a>'
				),
				'err'
	...
	public function handle_return( $log = false ) {
		...
		// Step 1: Find the invoice_id and determine if payment is MWPS or M1.
		if ( $payment_status || $transaction_type ) {
			...
			if ( $invoice_id ) {
				// BEST CASE:
				// 'invoice' is set in all regular MWPS subscriptions!
			...
			if ( ! $invoice_id ) {
				if ( ! empty( $_POST['custom'] ) ) {
					...
					if ( count( $infos ) > 2 ) {
						...
						$m1_sub_id  = intval( $infos[2] ); // Roughly equals MWPS membership->id.
		...
		if ( $transaction_type ) {
			switch ( $transaction_type ) {
				...
				case 'subscr_signup':
					...
					$notes_txn = __( 'Subscription has been created', 'memberdash' );
					$do_action = 'pay-free';
					break;

				case 'subscr_payment':
					// Payment was received
					$notes_txn = __( 'Payment successful', 'memberdash' );
					$do_action = 'pay-free';
					break;

				case 'recurring_payment':
					// Payment was received
					$notes_txn = __( 'Recurring payment successful', 'memberdash' );
					$do_action = 'pay-free';
					break;

				case 'subscr_modify':
					// Payment profile was modified
					$notes_txn = __( 'Subscription has been modified', 'memberdash' );
					break;

				case 'subscr_failed':
					// Payment profile was modified
					$notes_txn = __( 'Payment failed', 'memberdash' );
					$do_action = 'error';
					break;
				...
				case 'subscr_cancel':
					...
					$notes_txn      = __( 'Subscription has been canceled', 'memberdash' );
					$do_action      = 'cancel';
				...
				case 'recurring_payment_suspended':
					...
					$notes_txn      = __( 'Subscription has been suspended', 'memberdash' );
					$do_action      = 'cancel';
					$payment_status = false; // Skip the payment_status check.
					break;
				...
				case 'recurring_payment_suspended_due_to_max_failed_payment':
					...
					$notes_txn      = __( 'Subscription has failed', 'memberdash' );
					$do_action      = 'cancel';
					$payment_status = false; // Skip the payment_status check.
					break;
				...
				case 'new_case':
					...
					if ( ! empty( $_POST['buyer_additional_information'] ) ) {
						$notes_txn = sprintf(
							__( 'New Dispute filed: %s', 'memberdash' ),
							$_POST['buyer_additional_information']
						);
					} else {
						$notes_txn = __( 'New Dispute filed', 'memberdash' );
					}
				...
				case 'subscr_eot':
					 ...
					 */
					$notes_txn = __( 'Subscription ended', 'memberdash' );
					break;

				case 'recurring_payment_profile_created':
					$notes_txn = __( 'Recurring payment profile created', 'memberdash' );
					break;

				case 'recurring_payment_failed':
					$notes_txn = __( 'Recurring payment failed', 'memberdash' );
					$do_action = 'error';
					break;

				case 'recurring_payment_profile_cancel':
					$notes_txn = __( 'Recurring payment profile cancelled', 'memberdash' );
					break;

				case 'recurring_payment_expired':
					$notes_txn = __( 'Recurring payment profile expired', 'memberdash' );
					break;

				case 'recurring_payment_skipped':
					$notes_txn = __( 'Recurring payment profile skipped', 'memberdash' );
					break;
				...
				case 'recurring_payment_outstanding_payment':
					...
					if ( is_numeric( $os_amount ) && floatval( $os_amount ) ) {
						$notes_txn = sprintf(
							__( 'Outstanding payment of %s', 'memberdash' ),
							trim( $currency . ' ' . $os_amount )
						);
					} else {
						$notes_txn = __( 'Outstanding payment', 'memberdash' );
					}
					break;
				...
				case 'recurring_payment_outstanding_payment_failed':
					...
					if ( is_numeric( $os_amount ) && floatval( $os_amount ) ) {
						$notes_txn = sprintf(
							__( 'Collecting payment of %s failed', 'memberdash' ),
							trim( $currency . ' ' . $os_amount )
						);
					} else {
						$notes_txn = __( 'Collecting payment failed', 'memberdash' );
					}
					$do_action = 'error';
					break;
				...
				case 'send_money':
					if ( ! empty( $_POST['memo'] ) ) {
						$notes_txn = sprintf(
							__( 'Money sent for: %s', 'memberdash' ),
							$_POST['memo']
						);
					} else {
						$notes_txn = __( 'Money sent', 'memberdash' );
					}
					break;
				...
				default:
					...
					$notes_txn = sprintf(
						__( 'Not handling txn_type: %s', 'memberdash' ),
						$transaction_type
					);
					break;
			}
		...
		if ( $payment_status ) {
			switch ( $payment_status ) {
				...
				case 'reversed':
					$notes_pay = __( 'Last transaction has been reversed: Payment has been reversed (charge back).', 'memberdash' );
					$status    = MS_Model_Invoice::STATUS_DENIED;
					break;

				case 'refunded':
					$notes_pay = __( 'Last transaction has been reversed: Payment has been refunded.', 'memberdash' );
					$status    = MS_Model_Invoice::STATUS_DENIED;
					break;

				case 'denied':
					$notes_pay = __( 'Last transaction has been reversed: Payment Denied.', 'memberdash' );
					$status    = MS_Model_Invoice::STATUS_DENIED;
					$do_action = 'error';
					break;
				...
				case 'pending':
					...
					$notes_pay = __( 'Last transaction is pending.', 'memberdash' ) . ' ';

					switch ( $_POST['pending_reason'] ) {
						case 'address':
							$notes_pay .= __( 'No confirmed shipping address', 'memberdash' );
							break;

						case 'authorization':
							$notes_pay .= __( 'Funds not captured yet', 'memberdash' );
							break;

						case 'echeck':
							$notes_pay .= __( 'eCheck has not cleared yet', 'memberdash' );
							break;

						case 'intl':
							$notes_pay .= __( 'Pending approval by service provider', 'memberdash' );
							break;

						case 'multi-currency':
							$notes_pay .= __( 'Pending multi-currency process by service provider', 'memberdash' );
							break;

						case 'unilateral':
							$notes_pay .= __( 'No confirmed email address', 'memberdash' );
							break;

						case 'upgrade':
							$notes_pay .= __( 'Pending upgrade of PayPal account', 'memberdash' );
							break;

						case 'verify':
							$notes_pay .= __( 'Pending verification of PayPal account', 'memberdash' );
							break;

						default:
							$notes_pay .= __( 'Unknown reason', 'memberdash' );
							break;
					}
				...
				case 'in-progress':
					$notes_pay = sprintf(
						__( 'Not handling payment_status: %s', 'memberdash' ),
						$payment_status
					);
					break;
			}
		...
		// Step 2a: Check if the txn_id was already processed by MWPS.
		if ( $transaction_exists ) {
				...
				switch ( $do_action ) {
					case 'pay':
						...
						if ( $amount == $invoice->total ) {
							$notes_pay .= __( 'Payment successful', 'memberdash' );
						} else {
							$notes_pay .= __( 'Payment registered, though amount differs from invoice.', 'memberdash' );
						}
						break;
			...
			if ( 'm1' == $ext_type ) {
				 ...
				 * created in the PayPal account and not by M1/MWPS.
				 */
				...
				if ( ! empty( $notes_err ) ) {
					// Use the notes_err.
					$notes = $notes_err;
				} if ( ! empty( $notes ) ) { //phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedIf
