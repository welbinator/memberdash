...
class MS_Rule_Content_Model extends MS_Rule {
	...
	public function protect_content() {
		 ...
		 * This is the access granted to guests or memberships that do not define
		...
		} elseif ( true === $has_none ) { //phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedElseif
	...
	protected function get_rule_items() {
		if ( ! is_array( $this->_content ) ) {
			...
			$this->_content[ self::COMMENT_NO_ACCESS ] = (object) array(
				'label' => __( 'Comments: No Access', 'memberdash' ),
			);
			$this->_content[ self::COMMENT_READ ]      = (object) array(
				'label' => __( 'Comments: Read Only Access', 'memberdash' ),
			);
			$this->_content[ self::COMMENT_WRITE ]     = (object) array(
				'label' => __( 'Comments: Read and Write Access', 'memberdash' ),
			);
			$this->_content[ self::MORE_LIMIT ]        = (object) array(
				'label' => __( 'Hide "read more" content', 'memberdash' ),
			);
		}
