...
class MS_Rule_Shortcode_Model extends MS_Rule {
	 ...
	 * @var array
	 */
	protected static $membership_ids = array();
	...
	public function protect_content() {
		...
		if ( MS_Model_Addon::is_enabled( MS_Model_Addon::ADDON_SHORTCODE ) ) {
			...
			foreach ( $shortcode_tags as $shortcode => $callback_function ) {
	...
	public function do_protected_shortcode() {
		...
		if ( $msg ) {
			...
			$content = __( 'Shortcode content protected.', 'memberdash' );
		}
	 ...
	 *
	 * Call: self::PROTECT_CONTENT_SHORTCODE
	 *
			...
			if ( self::is_member_of( $membership_ids ) ) {
				...
				if ( $silent ) {
					...
					if ( ! empty( $msg ) ) {
						...
						$content         .= __( 'No access to members of: ', 'memberdash' );
			...
			if ( ! self::is_member_of( $membership_ids ) ) {
				...
				if ( $silent ) {
					...
					if ( ! empty( $msg ) ) {
						...
						$content         .= __( 'Content protected to members of: ', 'memberdash' );
						$content         .= implode( ', ', $membership_names );
					}
					$content .= '</div>';
				}
	...
	public static function debug_protect_content_shortcode( $atts, $content = '' ) {
		...
		if ( mslib3()->is_true( $access ) ) {
			$msg_access = __( 'Visible for members of', 'memberdash' );
			$alt_msg1   = __( 'Other users will see', 'memberdash' );
			$alt_msg2   = __( 'Other users will see nothing', 'memberdash' );
		} else {
			$msg_access = __( 'Hidden from members of', 'memberdash' );
			$alt_msg1   = __( 'Those users will see', 'memberdash' );
			$alt_msg2   = __( 'Those uses will see nothing', 'memberdash' );
		}
		...
		$code = sprintf(
			...
			$msg_access,
			implode( ' ', $memberships ),
			__( 'This information is only displayed for admin users', 'memberdash' )
		);
	 ...
	 * @return bool
	 */
	protected static function is_member_of( $ids ) {
		$result = false;
