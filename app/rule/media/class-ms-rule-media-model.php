...
class MS_Rule_Media_Model extends MS_Rule {
	...
	public static function get_protection_types() {
		...
		$example1 = MS_Helper_Utility::home_url( $mask . ( new Datetime( 'now', wp_timezone() ) )->format( '/Y/m/' ) . 'my-image.jpg' );
		$example2 = MS_Helper_Utility::home_url( $mask . '/ms_12345.jpg' );
		$example3 = MS_Helper_Utility::home_url( $mask . '/?ms_file=ms_12345.jpg' );
		$example1 = '<br /><small>' . __( 'Example:', 'memberdash' ) . ' ' . $example1 . '</small>';
		$example2 = '<br /><small>' . __( 'Example:', 'memberdash' ) . ' ' . $example2 . '</small>';
		$example3 = '<br /><small>' . __( 'Example:', 'memberdash' ) . ' ' . $example3 . '</small>';

		$protection_types = array(
			self::PROTECTION_TYPE_BASIC    => __( 'Basic protection (default)', 'memberdash' ) . $example1,
			self::PROTECTION_TYPE_COMPLETE => __( 'Complete protection', 'memberdash' ) . $example2,
			self::PROTECTION_TYPE_HYBRID   => __( 'Hybrid protection', 'memberdash' ) . $example3,
		);
	 ...
	 * @since 1.0.0
	 *
	 * @todo refactor hack to get extension.
	 *
	...
	private function output_file( $file ) {
		...
		if ( ! defined( 'MWPS_MEDIA_ETAG_DISABLED' ) ) {
			if ( ! defined( 'MWPS_MEDIA_ETAG' ) ) {
				define( 'MWPS_MEDIA_ETAG', 'mwps_media_addon_etag' );
			}
