...
class MS_Rule_Media_ListTable extends MS_Helper_ListTable_Rule {
	...
	public function __construct( $model ) {
		...
		$this->name['singular'] = __( 'Media File', 'memberdash' );
		$this->name['plural']   = __( 'Media Files', 'memberdash' );
	}

	public function get_columns() {
		$columns = array(
			'cb'        => true,
			'name'      => __( 'Title', 'memberdash' ),
			'access'    => true,
			'file_type' => __( 'Type', 'memberdash' ),
			'post_date' => __( 'Added', 'memberdash' ),
			'dripped'   => false,
		);
