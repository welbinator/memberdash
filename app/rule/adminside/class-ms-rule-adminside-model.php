...
class MS_Rule_Adminside_Model extends MS_Rule {
	 ...
	 * @var array
	 */
	protected static $denied_items = array();
	 ...
	 * http://premium.memberdash.org/forums/topic/multiple-membership-types-defaults-to-less-access-protected-content
	 *
	 * Relevant Action Hooks:
	 * - network_admin_menu
	 * - user_admin_menu
	...
	public function get_contents( $args = null ) {
		...
		$blacklist = array(
			...
			MS_Controller_Plugin::MENU_SLUG, // Main Membership menu item.
			// Membership sub-menu items
			...
			MS_Controller_Plugin::MENU_SLUG . '-reporting',
		...
		foreach ( $Items as $key => $item ) {
			if ( is_array( $filter->include ) ) {
				if ( ! in_array( $key, $filter->include ) ) {
					continue; }
