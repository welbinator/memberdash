...
class MS_Rule_ReplaceLocation_ListTable extends MS_Helper_ListTable_RuleMatching {
	...
	public function __construct( $model ) {
		...
		$this->name['singular'] = __( 'Menu Location', 'memberdash' );
		$this->name['plural']   = __( 'Menu Locations', 'memberdash' );
	...
	protected function get_column_label( $col ) {
		...
		switch ( $col ) {
			case 'item':
				$label = __( 'Menu Location', 'memberdash' );
				break;
			case 'match':
				$label = __( 'Show this menu to members', 'memberdash' );
				break;
		}
