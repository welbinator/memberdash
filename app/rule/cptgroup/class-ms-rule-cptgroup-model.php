...
class MS_Rule_CptGroup_Model extends MS_Rule {
	...
	public function protect_posts( &$wp_query ) {
		...
		if ( ! empty( $post_types ) // Only protect anything if post-type is known
			...
			foreach ( $post_types as $post_type ) {
				...
				// Do not protect special "Membership" or default WordPress content
				if ( in_array( $post_type, $excluded ) ) {
					$allow = true;

					// Do not protect if the post-type is published
				} elseif ( parent::has_access( $post_type ) ) {
					$allow = true; }
	...
	public function has_access( $id, $admin_has_access = true ) {
		...
		if ( in_array( $post_type, self::get_ms_post_types() ) ) {
			// Always allow access to Membership pages.
			$has_access = true;
	...
	public static function get_custom_post_types() {
		$args     = apply_filters(
			'ms_rule_cptgroup_model_get_post_types_args',
			array(
				'public'   => true,
				'_builtin' => false,
			)
