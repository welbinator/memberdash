...
class MS_View_Adminbar extends MS_View {
	...
	public function to_html() {
		...
		$details             = mslib3()->get_simulation_access();
		$denied_url          = false;
		...
		<div class="ms-sim-info <?php echo esc_attr( $toggle_state ); ?>">
			<div class="ms-sim-block">
				<h4 class="toggle-wrap">
					<?php esc_html_e( 'Simulation Overview', 'memberdash' ); ?>
				...
				<div class="ms-sim-body">
					...
					<table cellspacing="0" cellpadding="0" width="100%" border="0">
						<tr>
							<th><?php esc_html_e( 'View as', 'memberdash' ); ?></th>
							<td><?php MS_Helper_Html::html_element( $fields['membership_id'] ); ?></td>
						</tr>
						<?php if ( $this->data['datepicker'] ) : ?>
						<tr>
							<th><?php esc_html_e( 'View on', 'memberdash' ); ?></th>
							<td><?php MS_Helper_Html::html_element( $fields['simulate_date'] ); ?></td>
						</tr>
						<?php endif; ?>
						<tr>
							<th>&nbsp;</th>
							<td><button class="button"><?php esc_html_e( 'Update', 'memberdash' ); ?></button></td>
						</tr>
					</table>
					<?php
					...
					<h4 class="inside">
						<?php esc_html_e( 'Simulated Membership', 'memberdash' ); ?>
					</h4>
					<table cellspacing="0" cellpadding="0" width="100%" border="0" class="inside">
						<tr>
							<th><?php esc_html_e( 'Membership', 'memberdash' ); ?></th>
							<td style="white-space: nowrap"><?php echo esc_html( $mem->name ); ?></td>
						</tr>
						<tr>
							<th><?php esc_html_e( 'Type', 'memberdash' ); ?></th>
							<td><?php echo esc_html( $mem->get_type_description() ); ?></td>
						</tr>
						<tr>
							<th><?php esc_html_e( 'Start Date', 'memberdash' ); ?></th>
							<td><?php echo esc_html( $sim->start_date ); ?></td>
						</tr>
						<tr>
							<th><?php esc_html_e( 'Expire Date', 'memberdash' ); ?></th>
							<td><?php echo esc_html( $sim->expire_date ); ?></td>
						</tr>
						<?php if ( $this->data['datepicker'] ) : ?>
						<tr>
							<th><?php esc_html_e( 'Simulated Date', 'memberdash' ); ?></th>
						...
						<tr>
							<th><?php esc_html_e( 'Payment model', 'memberdash' ); ?></th>
							<td><?php echo esc_html( $pay_types[ $mem->payment_type ] ); ?></td>
						</tr>
						<tr>
							<th><?php esc_html_e( 'Payment details', 'memberdash' ); ?></th>
							<td><?php echo esc_html( wp_strip_all_tags( $sim->get_payment_description( null, true ) ) ); ?></td>
						</tr>
					</table>
				</div>
	...
	public function prepare_fields() {
		...
		foreach ( $memberships as $membership ) {
			if ( $base_id == $membership->id ) {
				$label = __( '- No membership / Visitor -', 'memberdash' );
			} else {
				$label = $membership->name;
				if ( ! $membership->active ) {
					$label .= ' ' . __( '(Inactive)', 'memberdash' );
				}
			}
		...
		$fields = array(
			'exit_button'     => array(
				...
				'value' => __( 'Exit Test Mode', 'memberdash' ),
	...
	protected function output_deciding_info( $denied_url, $membership_id, $rules ) {
		...
		if ( ! empty( $denied_url ) ) {
			...
			<?php
			printf(
				esc_html__( 'Access denied by %s', 'memberdash' ),
			...
			foreach ( $rules as $rule_type ) {
				printf(
					...
					esc_attr( $rule_title[ $rule_type ] ),
					esc_html__( 'Denied by Rule:', 'memberdash' )
				);
			}
			?>
		<?php } elseif ( $membership->is_valid() ) { ?>
			...
			<?php
			printf(
				esc_html__( 'Access granted by %s', 'memberdash' ),
				'<b>' . esc_html( $membership->name ) . '</b>'
			);
			?>
			</div>
			...
			foreach ( $rules as $rule_type ) {
				printf(
					...
					esc_attr( $rule_title[ $rule_type ] ),
					esc_html__( 'Allowed by Rule:', 'memberdash' )
				);
			}
			?>
			</ul>
		<?php } else { ?>
			...
			<?php esc_html_e( 'Unprotected', 'memberdash' ); ?>
			</div>
			<?php
		}
	}
	...
	protected function output_scripts() {
		...
		.ui-datepicker.memberdash-datepicker {
			padding: 0;
			border: none;
			background: transparent;
			z-index: 20;
		...
		.ui-datepicker.memberdash-datepicker .ui-datepicker-header,
		.ui-datepicker.memberdash-datepicker.ui-corner-all {
			border-radius: 0;
		}
		.ui-datepicker.memberdash-datepicker .ui-datepicker-prev,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-prev.ui-state-hover,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-next,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-next.ui-state-hover,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-title {
			line-height: 2.5em;
			height: 2.5em;
			padding: 0;
			border: 0;
		...
		.ui-datepicker.memberdash-datepicker .ui-datepicker-prev.ui-datepicker-prev-hover {
			top: 2px;
			left: 2px;
		}
		.ui-datepicker.memberdash-datepicker .ui-datepicker-next.ui-datepicker-next-hover {
			top: 2px;
			right: 2px;
		}
		...
		.ui-datepicker.memberdash-datepicker .ui-icon {
			width: 16px;
			height: 16px;
			text-indent: -99999px;
			overflow: hidden;
			background-repeat: no-repeat;
			background-image: url(<?php echo MS_Plugin::instance()->url; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>/app/assets/images/ui-icons-white.png);
			cursor: pointer;
		}
		.ui-datepicker.memberdash-datepicker .ui-datepicker-header {
			border: none;
		...
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody td a,
		.ui-datepicker.memberdash-datepicker td span,
		.ui-datepicker.memberdash-datepicker td a {
			padding: 8px;
			text-align: center;
			text-decoration: none;
		}
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar {
			padding: 0;
			background: #375D7A;
			width: 250px;
		}
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar thead th {
			padding: 3px 5px;
			font-size: 0.75em;
			font-weight: normal;
			color: #FFFFFF;
		}
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-state-default,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-widget-content .ui-state-default,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-widget-header .ui-state-default {
			border: 0;
			background: #223D52;
			color: #FFFFFF;
		}
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-state-active,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-widget-content .ui-state-active,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-widget-header .ui-state-active,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-state-hover,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-widget-content .ui-state-hover,
		.ui-datepicker.memberdash-datepicker .ui-datepicker-calendar tbody .ui-widget-header .ui-state-hover {
			background: #4F83AA;
		}
		.memberdash-datepicker {
