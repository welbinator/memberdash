...
class MS_View_Frontend_Payment extends MS_View {
	...
	public function to_html() {
		...
		$invoice->checkout_date = ( new Datetime( 'now', wp_timezone() ) )->format( 'Y-m-d H:i:s' );
		$invoice->save();
		...
		if ( ! MS_Model_Member::is_admin_user() ) {
			...
			if ( ! MS_Model_Addon::is_enabled( MS_Model_Addon::ADDON_MULTI_MEMBERSHIPS ) ) {
				...
				foreach ( $this->data['member']->subscriptions as $tmp_subscription ) {
					...
					if ( in_array( $tmp_subscription->status, $valid_status, true ) ) {
						$cancel_warning = __(
				...
				if ( 1 == count( $names ) ) {
					$cancel_warning = sprintf(
						__( 'When you complete this payment your Membership "%s" will be cancelled.', 'memberdash' ),
						$names[0]
					);
				} elseif ( 1 < count( $names ) ) {
					$cancel_warning = sprintf(
						__( 'When you complete this payment the following Memberships will be cancelled: %s.', 'memberdash' ),
						implode( ', ', $names )
					);
				}
			}
		...
		if ( $skip_form ) {
			 ...
			 */
			?>
			<script>window.location.href = '<?php echo esc_url_raw( add_query_arg( $args ) ); ?>';</script>
			<?php
			exit;
		}
		...
		$path = MS_Helper_Template::template_exists( 'membership-frontend-payment.php' );
		if ( $path ) {
			require $path;
		}
