...
class MS_View_Shortcode_Login extends MS_View {
	...
	public function to_html() {
		...
		if ( 'logout' === $form ) {
			...
			if ( $ajax_login ) {
				...
				$res_form .= wp_login_form(
					array(
						'echo'     => false,
						'redirect' => $redirect_login,
					)
				);
				$res_form .= '<br/><a href="' . wp_lostpassword_url( get_bloginfo( 'url' ) ) . '" title="' . __( 'Lost Password', 'memberdash' ) . '">' . __( 'Lost Password', 'memberdash' ) . '</a>';
			}
		...
		// Remove line breaks to bypass the "wpautop" filter.
	...
	private function login_header_html() {
		...
		<div class="ms-alert-box ms-alert-error ms-auth-header">
			<?php esc_html_e( 'Please log in to access this page.', 'memberdash' ); ?>
		</div>
			<?php
		endif;
	...
	private function login_form( $redirect_to = null ) {
		...
		$defaults = array(
			...
			'label_username' => __( 'Username', 'memberdash' ),
			'label_password' => __( 'Password', 'memberdash' ),
			'label_remember' => __( 'Remember Me', 'memberdash' ),
			'label_log_in'   => __( 'Log In', 'memberdash' ),
		...
		<form
			...
			<div class="form">
				...
				<?php echo apply_filters( 'login_form_top', '', $args ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<?php if ( 'top' === $nav_pos ) : ?>
					<?php
					if ( ! empty( $error_message ) ) {
						?>
							<div class="status"><?php echo $error_message; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
							<?php
					} else {
						?>
				...
				<?php echo apply_filters( 'login_form_middle', '', $args );//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<?php if ( 'top' === $nav_pos ) : ?>
					<div class="nav">
						<p><a class="lost" href="#lostpassword">
							<?php esc_html_e( 'Lost your password?', 'memberdash' ); ?>
						</a></p>
					</div>
				<?php endif; ?>
				...
				<?php echo apply_filters( 'login_form_bottom', '', $args ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			...
			<div class="nav">
				<p><a class="lost" href="#lostpassword"><?php esc_html_e( 'Lost your password?', 'memberdash' ); ?></a></p>
			<?php endif; ?>
			</div>
		</form>
		<?php
	...
	private function lostpass_form() {
		$defaults = array(
			'label_lost_username' => __( 'Username or E-mail', 'memberdash' ),
			'label_lostpass'      => __( 'Reset Password', 'memberdash' ),
		...
		<form
			...
			<div class="form">
				...
				<?php echo apply_filters( 'lostpass_form_top', '', $args ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<?php if ( 'top' === $nav_pos ) : ?>
					<div class="status" style="display:none"></div>
				<?php endif; ?>
				...
				<?php echo apply_filters( 'lostpass_form_middle', '', $args ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<?php if ( 'top' === $nav_pos ) : ?>
					<div class="nav">
						<p><a class="login" href="#login"><?php esc_html_e( 'Log in', 'memberdash' ); ?></a></p>
					</div>
				<?php endif; ?>
				<?php
				/**
				...
				<?php echo apply_filters( 'lostpass_form_bottom', '', $args ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<?php if ( 'bottom' === $nav_pos ) : ?>
				<div class="status" style="display:none"></div>
			</div>
			<div class="nav">
	...
	private function logout_form() {
		...
		$yourname = sprintf(
			__( 'You are logged in as %s.', 'memberdash' ),
		...
		if ( null === $Reset_Result ) {
			...
			if ( ! $user || is_wp_error( $user ) ) {
				...
				if ( $user && 'expired_key' == $user->get_error_code() ) {
					...
					$err_msg->add(
						...
						__( 'Sorry, this reset-key is not valid anymore. Please request a new reset email and try again.', 'memberdash' )
					);
				} else {
					$fatal_error = true;
					$err_msg->add(
						'password_invalid_key',
						__( 'Sorry, we did not find a valid reset-key. Please request a new reset email and try again.', 'memberdash' )
					);
				}
			} else {
				...
				if ( $pass1 && $pass1 != $pass2 ) {
					...
					$err_msg->add(
						...
						__( 'The passwords do not match, try again.', 'memberdash' )
					);
				}
			}
			...
			if ( $fatal_error && count( $err_msg->errors ) ) {
				...
				$Reset_Result = sprintf(
					...
					$url,
					__( 'Request a new password-reset key', 'memberdash' )
				);
			} elseif ( $pass1 ) {
				...
				$Reset_Result = sprintf(
					...
					__( 'Your Password has been reset.', 'memberdash' ),
					$url,
					__( 'Login with your new password', 'memberdash' )
				);
			} else {
				// This action is documented in wp-login.php
				do_action( 'validate_password_reset', $err_msg, $user );
				...
				if ( count( $err_msg->errors ) ) {
					printf(
						...
						implode( '<br>', $err_msg->get_error_messages() ) //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					);
				}
				?>
				<form name="resetpassform" id="resetpassform"
					...
					<p class="user-pass1-wrap">
						<label for="pass1"><?php esc_html_e( 'New password', 'memberdash' ); ?></label><br />
						<div class="wp-pwd">
							...
							<div id="pass-strength-result" class="hide-if-no-js" aria-live="polite"><?php esc_html_e( 'Strength indicator', 'memberdash' ); ?></div>
						</div>
					</p>
					<p class="user-pass2-wrap">
						<label for="pass2"><?php esc_html_e( 'Confirm new password', 'memberdash' ); ?></label><br />
						<input type="password" name="pass2" id="pass2" class="input" size="20" value="" autocomplete="off" />
					</p>

					<p class="description indicator-hint"><?php echo wp_get_password_hint(); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
					...
					<p class="submit">
						...
						<?php esc_html_e( 'Reset Password', 'memberdash' ); ?>
						</button>
					</p>
				</form>
				<?php
	...
	public function enqueue_scripts() {
		...
		mslib3()->ui->data(
			'ms_ajax_login',
			array(
				'loginmessage'   => __( 'Please log in to access this page.', 'memberdash' ),
				'resetmessage'   => __( 'Please enter your details to reset your password', 'memberdash' ),
				'ajaxurl'        => admin_url( 'admin-ajax.php', is_ssl() ? 'https' : 'http' ),
				'loadingmessage' => __( 'Please wait...', 'memberdash' ),
				'errormessage'   => __( 'Request failed, please try again.', 'memberdash' ),
			)
		);
	}
