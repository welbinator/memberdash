...
class MS_Controller_Shortcode extends MS_Controller {
	 ...
	 * Membership Page "Membership" is displayed.
	 *
	 * @since 1.0.0
	 */
		...
		$data              = apply_filters(
			...
			shortcode_atts(
				array(
					...
					'label_first_name' => __( 'First Name', 'memberdash' ),
					'label_last_name'  => __( 'Last Name', 'memberdash' ),
					'label_username'   => __( 'Choose a Username', 'memberdash' ),
					'label_email'      => __( 'Email Address', 'memberdash' ),
					'label_password'   => __( 'Password', 'memberdash' ),
					'label_password2'  => __( 'Confirm Password', 'memberdash' ),
					'label_register'   => __( 'Register My Account', 'memberdash' ),
					'hint_first_name'  => '',
					'hint_last_name'   => '',
					'hint_username'    => '',
					'hint_email'       => '',
					'hint_password'    => '',
					'hint_password2'   => '',
					'title'            => __( 'Create an Account', 'memberdash' ),
					'loginlink'        => true,
				),
				$atts
			)
	...
	public function membership_signup( $atts ) {
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					MS_Helper_Membership::MEMBERSHIP_ACTION_SIGNUP . '_text'    => __( 'Signup', 'memberdash' ),
					MS_Helper_Membership::MEMBERSHIP_ACTION_MOVE . '_text'      => __( 'Change', 'memberdash' ),
					MS_Helper_Membership::MEMBERSHIP_ACTION_CANCEL . '_text'    => __( 'Cancel', 'memberdash' ),
					MS_Helper_Membership::MEMBERSHIP_ACTION_RENEW . '_text'     => __( 'Renew', 'memberdash' ),
					MS_Helper_Membership::MEMBERSHIP_ACTION_PAY . '_text'       => __( 'Complete Payment', 'memberdash' ),
				),
				$atts
			)
		);
		...
		// If there are no memberships or subscriptions, show an error message.
		if ( empty( $memberships ) && empty( $data['subscriptions'] ) ) {
			// Show an error message.
			return $this->ms_note(
				array( 'type' => 'error' ),
				__( 'Sorry there are no memberships available for registration.', 'memberdash' )
			);
		}
		...
		if ( ! MS_Model_Addon::is_enabled( MS_Model_Addon::ADDON_MULTI_MEMBERSHIPS ) ) {
			...
			foreach ( $data['member']->subscriptions as $subscription ) {
				...
				if ( in_array( $subscription->status, $valid_status, true ) ) {
		...
		/**
		 * Membership Signup Shortcode View.
		 *
		 * @var MS_View_Shortcode_MembershipSignup $view
		 */
		$view = MS_Factory::create( 'MS_View_Shortcode_MembershipSignup' );

		$view->set_data(
			/**
			 * Filters the data for the Membership Signup Shortcode View.
			 *
			 * @since 1.0.0
			 *
			 * @param array<mixed>            $data       The data for the Membership Signup Shortcode View.
			 * @param MS_Controller_Shortcode $controller The controller object.
			 *
			 * @return array<mixed> The data for the Membership Signup Shortcode View.
			 */
			apply_filters(
	...
	public function membership_buy( $atts ) {
		...
		if ( ! empty( $id ) ) {
			...
			if ( ! $membership->is_active() ) {
				return __( 'Sorry! The membership you are trying to register is not active.', 'memberdash' );
			}
	...
	public function membership_details( $atts ) {
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					'id'    => 0,
					'label' => __( 'Membership details:', 'memberdash' ),
				),
				$atts
			)
		);
	...
	/**
	 * Display the "Membership" message.
	 *
	 * @since 1.0.0
	 *
	 * @param mixed[] $atts Shortcode attributes.
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					...
					'membership_title'        => __( 'Your Membership', 'memberdash' ),
					'membership_change_label' => __( 'Change', 'memberdash' ),
					'show_profile'            => true,
					'show_profile_change'     => true,
					'profile_title'           => __( 'Personal details', 'memberdash' ),
					'profile_change_label'    => __( 'Edit', 'memberdash' ),
					'show_invoices'           => true,
					'limit_invoices'          => 10,
					'show_all_invoices'       => true,
					'invoices_title'          => __( 'Invoices', 'memberdash' ),
					'invoices_details_label'  => __( 'View all', 'memberdash' ),
					'show_activity'           => true,
					'limit_activities'        => 10,
					'show_all_activities'     => true,
					'activity_title'          => __( 'Activities', 'memberdash' ),
					'activity_details_label'  => __( 'View all', 'memberdash' ),
				),
				$atts
			)
		);
	...
	public function membership_account_link( $atts ) {
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					'label' => __( 'Visit your account page for more information.', 'memberdash' ),
				),
				$atts
			)
		);
