...
class MS_Controller_Dialog extends MS_Controller {
	...
	public function ajax_lostpass() {
		...
		if ( empty( $_POST['user_login'] ) ) {
			$resp['error'] = __( 'Enter a username or e-mail address.', 'memberdash' );
		} elseif ( strpos( $_POST['user_login'], '@' ) ) {
			$user_data = get_user_by( 'email', trim( $_POST['user_login'] ) );
			if ( empty( $user_data ) ) {
				$resp['error'] = __( 'There is no user registered with that email address.', 'memberdash' );
			}
		} else {
		...
		if ( ! $user_data ) {
			$resp['error'] = __( 'Invalid username or e-mail.', 'memberdash' );
			$this->respond( $resp );
		}
		...
		if ( ! $allow ) {
			$resp['error'] = __( 'Password reset is not allowed for this user', 'memberdash' );
			$this->respond( $resp );
		...
		if ( ! apply_filters( 'ms_sent_reset_password_email', false ) ) {
			...
			$message = sprintf(
				__( 'Someone has requested a password reset for the following account: %1$sIf this was a mistake, just ignore this email and nothing will happen.%2$s %3$s', 'memberdash' ),
				"\r\n\r\n" . network_home_url( '/', $schema ) . "\r\n\r\n" .
				sprintf( __( 'Username: %s', 'memberdash' ), $user_login ) . "\r\n\r\n",
				"\r\n\r\n" . __( 'To reset your password, visit the following address:', 'memberdash' ) . "\r\n",
				"\r\n<" . $reset->url . ">\r\n"
			);
			...
			$title   = sprintf( __( '[%s] Password Reset', 'memberdash' ), $blogname );
			$title   = apply_filters( 'retrieve_password_title', $title, $user_login, $user_data );
			$message = apply_filters( 'retrieve_password_message', $message, $user_login, $reset->url, $user_data );

			if ( $message && ! wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
				$resp['error'] = __( 'The e-mail could not be sent.', 'memberdash' ) . '<br />' .
					__( 'Possible reason: your host may have disabled the mail() function.', 'memberdash' );
			} else {
				$resp['success'] = __( 'Check your e-mail for the confirmation link.', 'memberdash' );
			}
		} else {
			$resp['success'] = __( 'Check your e-mail for the confirmation link.', 'memberdash' );
		}
