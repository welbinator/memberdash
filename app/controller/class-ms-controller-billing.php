...
class MS_Controller_Billing extends MS_Controller {
	...
	public function ajax_link_transaction() {
		...
		echo $resp; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		exit;
	}
	...
	public function ajax_link_data_transaction() {
		...
		if ( self::validate_required( $fields ) && $this->verify_nonce() ) {
			...
			if ( 'subscriptions' == $type ) {
				...
				$resp[0]  = __( 'Select a subscription', 'memberdash' );
				$active   = array();
				$inactive = array();
				foreach ( $member->subscriptions as $subscription ) {
					...
					$line = sprintf(
						__( 'Membership: %1$s, Base price: %2$s', 'memberdash' ),
						$membership->name,
						$price
					);
				...
				if ( ! count( $active ) && ! count( $inactive ) ) {
					$resp[0] = __( 'No subscriptions found', 'memberdash' );
				} else {
					if ( count( $active ) ) {
						$resp[ __( 'Active Subscriptions', 'memberdash' ) ] = $active;
					}
					if ( count( $inactive ) ) {
						$resp[ __( 'Expired Subscriptions', 'memberdash' ) ] = $inactive;
					}
				}
				...
				$resp[0] = __( 'Select an invoice', 'memberdash' );
				$unpaid  = array();
				$paid    = array();
				foreach ( $invoices as $invoice ) {
					$line = sprintf(
						__( 'Invoice: %1$s from %2$s (%3$s)', 'memberdash' ),
				...
				if ( ! count( $unpaid ) && ! count( $paid ) ) {
					$resp[0] = __( 'No invoices found', 'memberdash' );
				} else {
					if ( count( $unpaid ) ) {
						$resp[ __( 'Unpaid Invoices', 'memberdash' ) ] = $unpaid;
					}
					if ( count( $paid ) ) {
						$resp[ __( 'Paid Invoices', 'memberdash' ) ] = $paid;
					}
				}
			}
		}
	...
	public function enqueue_scripts() {
		...
		if ( isset( $_GET['action'] ) && 'edit' == $_GET['action'] ) {
			...
			if ( 'logs' == $module || 'matching' == $module ) {
				...
				$data['lang']      = array(
					'link_title' => __( 'Link Transaction', 'memberdash' ),
				);
			}
		}
