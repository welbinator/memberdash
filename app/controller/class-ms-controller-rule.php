...
class MS_Controller_Rule extends MS_Controller {
	...
	/**
	 * Handle Ajax update rule-match action.
	 *
	 * Related Action Hooks:
	...
	public function ajax_action_update_matching() {
		...
		if ( $this->_resp_ok() && ! $this->verify_nonce() ) {
			$this->_resp_err( 'update-matching-01' ); }
		if ( $this->_resp_ok() && ! self::validate_required( $required ) ) {
			$this->_resp_err( 'update-matching-02' ); }
		if ( $this->_resp_ok() && ! self::validate_required( $isset, 'POST', false ) ) {
			$this->_resp_err( 'update-matching-03' ); }
	...
	public function ajax_action_update_dripped() {
		...
		if ( ! $this->verify_nonce( 'inline' ) ) {
			$this->_resp_err( __( 'Invalid Nonce', 'memberdash' ) );
		} elseif ( ! $this->is_admin_user() ) {
			$this->_resp_err( __( 'Access Denied', 'memberdash' ) );
		} elseif ( ! self::validate_required( $fields, 'POST', false ) ) {
			$this->_resp_err( __( 'Missing Data', 'memberdash' ) );
		}
		...
		if ( $this->_resp_ok() ) {
			...
			foreach ( $ids as $id ) {
				if ( empty( $_POST[ 'ms_' . $id ] ) ) {
					continue; }
				$data = mslib3()->array->get( $_POST[ 'ms_' . $id ] );
				mslib3()->array->equip( $data, 'dripped_type', 'date', 'delay_unit', 'delay_type' );
		...
		if ( MS_Helper_Membership::MEMBERSHIP_MSG_UPDATED == $msg ) {
			// If everything went well then get a refreshed version of the list-table.
			$GLOBALS['hook_suffix'] = 'memberdash_page_' . MS_Controller_Plugin::MENU_SLUG . '-protection';
			...
			if ( $table ) {
				...
				if ( isset( $items[ $item_id ] ) ) {
					echo $table->display_rows( array( $items[ $item_id ] ) ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
		} else {
			$msg .= $this->_resp_code();
	...
	public function edit_rule_manager( $rule_type ) {
		...
		do_action( 'ms_controller_rule_edit_rule_manager', $rule_type, $this );
