...
class MS_Controller_Communication extends MS_Controller {
	...
	public function __construct() {
		...
		// Observe all MWPS events and send/enqueue messages if needed.
		$this->add_action(
			'ms_model_event',
			'process_event',
			10,
		...
		// Log emails that were sent via MWPS.
		$this->add_action(
		...
		if ( $this->is_admin_user() && $this->verify_nonce() ) {
			...
			if ( self::validate_required( array( 'comm_type' ) )
				...
				$redirect = esc_url_raw(
					add_query_arg(
						array(
							'comm_type' => rawurlencode( $_POST['type'] ),
							'msg'       => $msg,
						)
					)
				);
	...
	public function ajax_action_update_communication() {
		...
		echo esc_js(
			apply_filters(
				'ms_controller_communication_ajax_action_update_communication_msg',
				$msg,
				$this
			)
		);
	 ...
	 * @param  MS_Model_Relationship  $subscription Related subscription object.
	 */
	public function log_sent_message( $sent, $recipients, $subject, $message, $headers, $comm, $subscription ) {
		...
		$log->trace           = wp_json_encode( $trace );
	 ...
	 */
	public static function add_mce_buttons() {
