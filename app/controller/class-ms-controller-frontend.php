...
/**
 * Controller for Membership/User registration.
 *
 * @since 1.0.0
 */
class MS_Controller_Frontend extends MS_Controller {
	...
	/**
	 * Whether Membership will handle the registration process or not.
	 ...
	 * @var bool
	 */
	public static $handle_registration = true;
	...
	public function __construct() {
		...
		if ( MS_Plugin::is_enabled() ) {
			 ...
			 *
			 * Set the filter response to FALSE to stop Membership from
	 ...
	 * current user is registered to. This allows web designers to adjust layout
	 ...
	 * If Membership handles registration we can provide the registration
	 ...
	 */
	public static function get_registration_url( $step = null ) {
	...
	public function register_form( $content ) {
		...
		if ( ! MS_Model_Member::can_register() ) {
			return __( 'Registration is currently not allowed.', 'memberdash' );
		}
	...
	public function verification_notification( $content ) {
		return apply_filters(
			...
			__( 'Please check your email for further instructions to verify your email.', 'memberdash' ),
			$content,
			$this
		);
	}
	...
	public function payment_table( $content ) {
		...
		if ( ! empty( $_POST['ms_relationship_id'] ) ) {
			...
			$move_from_id  = isset( $_REQUEST['move_from_id'] )
								? strval( sanitize_text_field( wp_unslash( $_REQUEST['move_from_id'] ) ) )
								: '';
		...
		/**
		 * Frontend payment view.
		 *
		 * @var MS_View_Frontend_Payment $view
		 */
		$view = MS_Factory::load( 'MS_View_Frontend_Payment' );

		$view->set_data(
			/**
			 * Filters the data for the frontend payment view.
			 *
			 * @since 1.0.0
			 *
			 * @param array<mixed>           $data          The data for the frontend payment view.
			 ...
			 * @param MS_Controller_Frontend $controller    The frontend controller.
			 *
			 * @return array<mixed> The data for the frontend payment view.
			 */
			apply_filters(
	...
	public function user_account_manager() {
		...
		switch ( $action ) {
			...
			case self::ACTION_VIEW_ACTIVATEACCOUNT:
				...
				$user_id          = intval( MS_Model_Member::verification_account_id( $verification_key ) );
	...
	public function enqueue_scripts() {
		...
		if ( $load_on_front_pages ) {
			$data = array(
				...
				'cancel_msg' => __( 'Are you sure you want to cancel?', 'memberdash' ),
			);
	...
	public function login_message( $message ) {
		if ( isset( $_GET['ms_error'] ) ) {
			$msg      = __( 'Account not verified. Please check your email for a verification link', 'memberdash' );
