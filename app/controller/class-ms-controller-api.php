 ...
 *
 *     class My_Membership_Implementation {
 *         protected $api = null;
 *
 ...
 *             static $Inst = null;
 *             if ( null === $Inst ) {
 *                 $Inst = new My_Membership_Implementation();
 *             }
 *         }
 *
 ...
 *         }
 *
 *         // Function is only run when Membership is present + active.
 ...
 *     }
 *     My_Membership_Implementation::setup();
 *
 * ----------------
 *
 ...
 *
 * Different ways to access the MWPS API object:
 *
...
class MS_Controller_Api extends MS_Hooker {
	...
	/**
	 * A reference to the Membership settings object.
	 *
	 * @since 1.0.0
	 * @api
	...
	protected function __construct() {
		 ...
		 * Membership is loaded and the API was initialized.
		 *
		 * Example:
		 *   if ( apply_filters( 'ms_active', false ) ) { ... }
		 *
		...
		/**
		 * Notify other plugins that Membership is ready.
		 *
		 * @since 1.0.0
		 */
	 ...
	 *
	 * Other plugins can store and access custom settings for each membership:
	 *
	 ...
	 * payment via the MWPS checkout page.
	 *
	 * @since 1.0.0
	 * @param  int        $user_id The User-ID.
	 * @param  int|string $membership_id The membership-ID or name/slug.
	...
	public function list_memberships( $list_all = false ) {
		...
		if ( ! $list_all ) {
			foreach ( $list as $key => $item ) {
				if ( ! $item->active ) {
					unset( $list[ $key ] ); } elseif ( $item->private ) {
					unset( $list[ $key ] ); }
			}
		}
	...
	/**
	 * Create your own payment gateway and hook it up with Membership by using
	 * this function!
	 *
	 ...
	 *     /*
	 *      * Plugin name: Membership Mygateway
	 *      * /
	 *
	 ...
	 * for Membership! Implementing the payment logic is up to you - you can
	...
	/**
	 * Membership has a nice integrated debugging feature. This feature can be
...
if ( ! function_exists( 'ms_api' ) ) {
	 ...
	 * cause problems because other parts of MWPS might not be completely
	 * initialized.
	 *
	 * @since 1.0.0
	 * @api
