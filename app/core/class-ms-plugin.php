...
class MS_Plugin {
	...
	public function __construct() {
		...
		$this->id      = MEMBERDASH_PLUGIN;
		$this->name    = MEMBERDASH_PLUGIN_NAME;
		$this->version = MEMBERDASH_VERSION;
		$this->file    = MEMBERDASH_PLUGIN_FILE;
		$this->dir     = MEMBERDASH_PLUGIN_DIR;
		$this->url     = plugin_dir_url( MEMBERDASH_PLUGIN_FILE );
		...
		add_filter(
			'plugin_action_links_' . MEMBERDASH_PLUGIN,
			array( $this, 'plugin_settings_link' )
		);

		add_filter(
			'network_admin_plugin_action_links_' . MEMBERDASH_PLUGIN,
			array( $this, 'plugin_settings_link' )
		);
	...
	public function plugin_activation() {
		update_option( 'ms_activation_redirect', true );

		// Prevent recursion during plugin activation.
		$refresh = get_option( 'memberdash_refresh_url_rules' );

		if ( $refresh ) {
			return;
		}
		/**
		 * The MS Licensing object.
		 *
		 * @var MS_Licensing $ms_licensing
		 */
		$ms_licensing = MS_Factory::load( 'MS_Licensing' );
		$ms_licensing->maybe_automate_activate();
		// Update the Membership database entries after activation.
	...
	/**
	 * Redirect to set up in the first time plugin activation
	 *
	 * @return void
	 */
	public function redirect_to_setup() {
		if ( wp_doing_ajax() ) {
			return; // Don't redirect AJAX calls.
		}

		$activation_option = get_option( 'ms_activation_redirect' );
		$current_step      = MS_Factory::create( 'MS_Controller_Plugin' )->controllers['membership']->get_step();
		if ( $activation_option && MS_Controller_Membership::STEP_WELCOME_SCREEN === $current_step ) {
	...
	public function check_update() {
		// no need to check nonce here, it just a condition check.
		//phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( wp_doing_ajax() && ( empty( $_POST['action'] ) || 'update-plugin' !== $_POST['action'] ) ) {
			return; // Don't check AJAX calls unless it is plugin update.
		}
		MS_Factory::load( 'MS_Licensing' )->check();
	}
	 ...
	 */
	public static function flush_rewrite_rules( $url = false ) {
		...
		update_option( 'memberdash_refresh_url_rules', true );
	...
	public function maybe_flush_rewrite_rules() {
		...
		delete_option( 'memberdash_refresh_url_rules' );
	...
	/**
	 * This function initializes the api property for easy access to the plugin
