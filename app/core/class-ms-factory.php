...
class MS_Factory {
	 ...
	 * @var   array
	 */
	private static $Prev_Blog_Id = array();
	 ...
	 * @param mixed  $init_arg Parameter arguments.
	 *
	 * @throws Exception Exception is the class doesn't exist or is not found.
	 *
	...
	public static function create( $class, $init_arg = null ) {
		...
		if ( in_array( $class, $singletons ) ) {
			_doing_it_wrong(
				'MS_Factory::create()',
				'This class is a singleton and should be fetched via MS_Factory::load() -> ' . esc_attr( $class ),
				'1.0.0'
			);
		}
	 ...
	 * @param string $context  The class context.
	 *
	 ...
	 * @param any    $obj
	 */
	public static function set_singleton( $obj, $key = null, $model_id = null ) {
		$class = get_class( $obj );
	 ...
	 *
	 * @since 1.0.0
	 * @param  MS_Hook &$obj Any Membership object to initialize.
	 */
	private static function prepare_obj( &$obj ) {
	...
	protected static function load_from_wp_user( $model, $user_id, $name = null ) {
		...
		if ( $cache ) {
			...
			if ( ! empty( $wp_user->ID ) ) {
				 ...
				 *
				 * @since 1.0.0
				 * @param  string $name The default display name used by MWPS.
	 ...
	 * @return array
	 */
	public static function serialize_model( &$model ) {
		$data   = array();
		$ignore = array();
		$class  = get_class( $model );
	 ...
	 * @since 1.0.0
	 */
	public static function select_blog( $site_id = null ) {
		global $wpdb;
