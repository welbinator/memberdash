...
/**
 * Class MS_Rule. Membership Rule Parent class.
 */
class MS_Rule extends MS_Model {
	...
	public static function rule_factory( $rule_type, $membership_id, $subscription_id ) {
		...
		if ( isset( $rule_types[ $rule_type ] ) ) {
			...
			$rule                   = MS_Factory::load( $class, $membership_id, MS_Helper_Cast::to_string( $subscription_id ) );
	 ...
	 * - For network-wide protection the $key has format "blog_id:rule_type".
	 *
	 * @since 1.0.0
	 ...
	 *
	 * If the query targeted multiple post_types at once, then an array of
	...
	public static function get_post_type( $wp_query ) {
		...
		if ( empty( $post_type ) ) {
			// The WP_Query does not explicitly specify a post_type. Guess.
	 ...
	 *               admin status and check protection rules normally.
	 *
	...
	public function has_access( $id, $admin_has_access = true ) {
		...
		if ( $this->is_base_rule ) {
			...
			// Apply dripped-content rules if necessary.
			...
			switch ( $drip_data['type'] ) {
				...
				case MS_Model_Rule::DRIPPED_TYPE_FROM_REGISTRATION:
					...
					if ( 0 == $drip_data['delay_unit'] ) {
						$desc = '<b>' . __( 'Instantly', 'memberdash' ) . '</b>';
					} elseif ( 1 == $drip_data['delay_unit'] ) {
						$desc = sprintf(
							__( 'After <b>%1$s %2$s</b>', 'memberdash' ),
	 ...
	 *     @type int $restricted The Membership count.
	 * }
	 */
	...
	public function protect_undefined_items( $base_rule ) {
		if ( $base_rule->rule_type != $this->rule_type ) {
			return; }
	...
	public function get_exclude_include( $args ) {
		...
		switch ( $status ) {
			...
			default:
				// If not visitor membership, just show all Membership
