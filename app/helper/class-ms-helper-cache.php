...
/**
 * Helper for cache related functions.
 *
 * @since 1.0.0
 */
class MS_Helper_Cache extends MS_Helper {
	...
	public static function generate_cache_key( $name, $args = null ) {
		if ( ! empty( $args ) ) {
			...
			$name = $name . '_' . md5( wp_json_encode( $args ) );
		}
	...
	public static function get_transient( $key, $default_enable = false ) {
		$results = self::get_cache( $key, self::CACHE_GROUP );
	...
	/**
	 * Wrapper for wp_cache_set in MWPS.
	 *
	...
	/**
	 * Wrapper for wp_cache_get function in MWPS.
	 *
	 ...
	 *                            Disambiguated a return of false, a storable value. Default null.
	 *
	 * @since 1.0.0
	 *
