...
class MS_Helper_Membership extends MS_Helper {
	...
	public static function get_admin_messages( $msg = 0 ) {
		$messages = apply_filters(
			...
			array(
				self::MEMBERSHIP_MSG_ADDED              => __( 'You have successfully set up your <b>%s</b> Membership.', 'memberdash' ),
				self::MEMBERSHIP_MSG_DELETED            => __( 'Membership deleted.', 'memberdash' ),
				self::MEMBERSHIP_MSG_UPDATED            => __( 'Membership <b>%s</b> updated.', 'memberdash' ),
				self::MEMBERSHIP_MSG_ACTIVATION_TOGGLED => __( 'Membership activation toggled.', 'memberdash' ),
				self::MEMBERSHIP_MSG_STATUS_TOGGLED     => __( 'Membership status toggled.', 'memberdash' ),
				self::MEMBERSHIP_MSG_BULK_UPDATED       => __( 'Memberships bulk updated.', 'memberdash' ),
				self::MEMBERSHIP_MSG_NOT_ADDED          => __( 'Membership not added.', 'memberdash' ),
				self::MEMBERSHIP_MSG_NOT_DELETED        => __( 'Membership not deleted.', 'memberdash' ),
				self::MEMBERSHIP_MSG_NOT_UPDATED        => __( 'Membership not updated.', 'memberdash' ),
				self::MEMBERSHIP_MSG_ACTIVATION_NOT_TOGGLED => __( 'Membership activation not toggled.', 'memberdash' ),
				self::MEMBERSHIP_MSG_STATUS_NOT_TOGGLED => __( 'Membership status not toggled.', 'memberdash' ),
				self::MEMBERSHIP_MSG_BULK_NOT_UPDATED   => __( 'Memberships bulk not updated.', 'memberdash' ),
				self::MEMBERSHIP_MSG_PARTIALLY_UPDATED  => __( 'Memberships partially updated. Some fields could not be changed after members have signed up.', 'memberdash' ),
			)
		);
	...
	public static function print_admin_message() {
		...
		$msg = self::get_admin_messages( $msg );
		if ( $msg ) {
	...
	public static function get_admin_message( $args = null, $membership = null ) {
		...
		$msg = self::get_admin_messages( $msg_id );
		if ( $msg ) {
			if ( ! empty( $args ) ) {
	...
	public static function get_admin_title() {
		...
		if ( self::MEMBERSHIP_MSG_ADDED == $msg ) {
			$title = __( 'Congratulations!', 'memberdash' );
		}
	...
	/**
	 * Displays a PopUp to the user that shows a summary of the setup wizard
	...
	public static function show_setup_note( $membership ) {
		...
		$popup['title']  = sprintf(
			...
			__( 'Congratulations!', 'memberdash' ),
			sprintf(
				__( 'You have successfully set up your <b>%1$s</b> Membership.', 'memberdash' ),
				$membership->name
			)
		);
		$popup['modal']  = true;
		...
		$popup['body'] .= sprintf(
			...
			__( 'Set-up Access Levels', 'memberdash' ),
			__( 'Finish', 'memberdash' )
		);
