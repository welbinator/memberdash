...
class MS_Helper_Utility extends MS_Helper {
	...
	public static function array_intersect_assoc_deep( &$arr1, &$arr2 ) {
		...
		foreach ( $intersections as $key ) {
			$result = self::array_intersect_assoc_deep(
				$arr1[ $key ],
				$arr2[ $key ]
			);
	 ...
	 * @return string
	 */
	public static function hsl2web( $h, $s, $l ) {
		$r = 0;
		$g = 0;
		$b = 0;
	 ...
	 * @return bool
	 */
	public static function is_current_url( $url, $ignore = array( 'paged' ) ) {
		static $Cur_url = null;
	 ...
	 * @return object
	 */
	public static function array_to_object( $array, $type ) {
		if ( is_array( $array ) ) {
			return (object) $array;
		}
	...
	public static function asset_link( $link, $ext = 'js' ) {
		// if the link contains the '.min' string then we assume it's already minified.
		if ( strpos( $link, '.min' ) !== false ) {
			return $link . '.' . $ext;
		}

		// Use minified libraries if MEMBERDASH_UNMINIFIED|SCRIPT_DEBUG is turned off.
		if ( ( defined( 'MEMBERDASH_UNMINIFIED' ) && MEMBERDASH_UNMINIFIED ) || ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ) {
			return $link . '.' . $ext;
		}

		return trailingslashit( dirname( $link ) ) . 'min/' . basename( $link ) . '.min.' . $ext;
	}
