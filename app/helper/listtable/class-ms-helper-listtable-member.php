...
class MS_Helper_ListTable_Member extends MS_Helper_ListTable {
	 ...
	 * @var array
	 */
	public static $memberships = null;
	...
	public function get_columns() {
		$columns = array(
			...
			'user_id'    => __( 'ID', 'memberdash' ),
			'username'   => __( 'Username', 'memberdash' ),
			'email'      => __( 'E-mail', 'memberdash' ),
	...
	public function column_infos( $member ) {
		...
		$html = sprintf(
			...
			esc_attr( wp_json_encode( $dialog_data ) )
		);
	...
	public function column_membership( $member ) {
		if ( MS_Model_Member::is_admin_user( $member->id ) ) {
			$html = '<b>' . __( 'Admin User', 'memberdash' ) . '</b>';
		} else {
	 ...
	 *     @type mixed $description The action description.
	 * }
	 */
	public function get_bulk_actions() {
		...
		$bulk_actions  = array(
			'drop-all'     => __( 'Drop all Memberships', 'memberdash' ),
			$protect_key   => array(),
			$unprotect_key => array(),
		);
	...
	public function searchbox_filters() {
		...
		$search_options = array(
			...
			'field_options' => array(
				'username'   => __( 'Username / E-mail', 'memberdash' ),
				'nickname'   => __( 'Nickname', 'memberdash' ),
				'first_name' => __( 'First Name', 'memberdash' ),
				'last_name'  => __( 'Last Name', 'memberdash' ),
			),
		);
	...
	public function get_views() {
		...
		$views['label'] = array(
			'label' => __( 'Subscription Status:', 'memberdash' ),
		);
		...
		$views['active']             = array(
			'url'   => $url,
			'label' => __( 'Active subscription', 'memberdash' ),
			'count' => $count,
		);
