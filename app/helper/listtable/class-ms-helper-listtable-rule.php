...
class MS_Helper_ListTable_Rule extends MS_Helper_ListTable {
	...
	/**
	 * Holds the human readable name of the rule title.
	 *
	 * @since 1.0.0
	 * @var array
	 */
	...
	public function __construct( $model ) {
		...
		$this->name['singular']       = __( 'Item', 'memberdash' );
		$this->name['plural']         = __( 'Items', 'memberdash' );
		$this->name['default_access'] = __( 'Everyone', 'memberdash' );
	...
	public function get_columns() {
		return apply_filters(
			...
			array(
				...
				'content'   => __( 'Content', 'memberdash' ),
				'rule_type' => __( 'Rule type', 'memberdash' ),
				'dripped'   => __( 'Dripped Content', 'memberdash' ),
			)
		);
	}
	...
	public function get_bulk_actions() {
		$protect_key   = __( 'Add Membership', 'memberdash' );
		$unprotect_key = __( 'Drop Membership', 'memberdash' );
		$bulk_actions  = array(
			'rem-all'      => __( 'Drop all Memberships', 'memberdash' ),
			$protect_key   => array(),
			$unprotect_key => array(),
		);

		$memberships = MS_Model_Membership::get_membership_names();
		$txt_add     = __( 'Add: %s', 'memberdash' );
		$txt_rem     = __( 'Drop: %s', 'memberdash' );
	...
	public function prepare_items() {
		...
		if ( $is_dripped ) {
			$this->_column_headers[0]['dripped'] = __( 'Reveal Content', 'memberdash' );
		} else {
			unset( $this->_column_headers[0]['dripped'] );
		}

		if ( isset( $this->_column_headers[0]['access'] ) ) {
			$this->_column_headers[0]['access'] = __( 'Who Has Access', 'memberdash' );
		}
	...
	/**
	 * Returns the membership of the current view.
	 *
	 * @since 1.0.0
	 * @return bool
	 */
	...
	public function column_access( $item, $column_name ) {
		...
		$public = array(
			...
			'after' => __( 'Modify Access', 'memberdash' ),
			'class' => 'ms-empty-note',
		);
		if ( ! empty( $rule->rule_value ) && array_key_exists( $item->id, $rule->rule_value ) ) {
			$public['value'] = __( 'None (assigned membership was deleted)', 'memberdash' );
		}
	...
	public function column_dripped( $item, $column_name ) {
			...
			foreach ( $Dripped_memberships as $membership ) {
				...
				if ( ! empty( $rule->dripped[ $item->id ] ) ) {
					if ( empty( $label ) ) {
						...
						// Multiple dripped memberships. Display placeholder text.
						$label = '';
						break;
					}
				}
		...
		ob_start();
		?>
		<a href="#" class="editinline"><?php echo $label;  //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a>
			...
			foreach ( $Dripped_memberships as $membership ) {
				...
				if ( ! empty( $rule->dripped[ $item->id ] ) ) {
					...
					printf(
						...
						esc_attr( $data['type'] ),
						esc_attr( $data['date'] ),
						esc_attr( $data['delay_unit'] ),
						esc_attr( $data['delay_type'] ),
						esc_attr( $offset ),
						esc_attr( $number )
					);
				}
	...
	protected function inline_edit() {
		...
		$field_date = array(
			...
			'name'        => 'date',
			'placeholder' => __( 'Date', 'memberdash' ) . '...',
		);
		...
		$field_delay_type = array(
			...
			'after'         => __( 'after subscription', 'memberdash' ),
		);
		...
		<div>
			<h4>
				...
				<?php esc_html_e( 'Dripped Content Settings', 'memberdash' ); ?>
			</h4>
		</div>
		<fieldset>
		...
		<div class="dripped-form cf no-auto-init hidden">
			...
			<div class="drip-col col-3">
				<div class="drip-option <?php echo esc_attr( MS_Model_Rule::DRIPPED_TYPE_INSTANTLY ); ?>">
					<?php esc_html_e( 'Instantly', 'memberdash' ); ?>
				</div>
	...
	public function list_head() {
		...
		if ( empty( $_GET['membership_id'] ) ) {
			if ( empty( $_GET['status'] ) ) {
				$title = __( 'Showing <b>All</b> %1$s', 'memberdash' );
			} elseif ( MS_Model_Rule::FILTER_NOT_PROTECTED == $_GET['status'] ) {
				$title = __( 'Showing All <b>Unprotected</b> %1$s', 'memberdash' );
			} elseif ( MS_Model_Rule::FILTER_PROTECTED == $_GET['status'] ) {
				$title = __( 'Showing All <b>Protected</b> %1$s', 'memberdash' );
			}
		} else {
			$membership = MS_Factory::load( 'MS_Model_Membership', $_GET['membership_id'] );

			if ( empty( $_GET['status'] ) ) {
				$title = __( 'Showing <b>All</b> %1$s for %2$s', 'memberdash' );
			} elseif ( MS_Model_Rule::FILTER_NOT_PROTECTED == $_GET['status'] ) {
				$title = __( 'Showing All %1$s that are <b>not protected</b> by %2$s', 'memberdash' );
			} elseif ( MS_Model_Rule::FILTER_PROTECTED == $_GET['status'] ) {
				$title = __( 'Showing All %1$s that are <b>protected</b> by %2$s', 'memberdash' );
			}
		...
		printf( '<h3 class="ms-list-title">%1$s</h3>', $title ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	...
	public function get_views() {
		...
		$views['public'] = array(
			'url'   => $public_url,
			'label' => __( 'Unprotected', 'memberdash' ),
