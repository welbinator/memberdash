 ...
 *        - gateway         [admin|free|manual|paypalstandard|stripe]
 *        - start           <yyyy-mm-dd>
 *        - expire          <yyyy-mm-dd>
 *        - trial_finished  <bool>
 ...
 *
 * - settings     <array>
 *    - global_payment_settings  <array>  Global payment settings
 *        - <option_name>  <option_value>  Configuration options
 *
 * @since 1.0.0
 */
class MS_Model_Export_Settings extends MS_Model {
	...
	public function process() {
		...
		// Export the base membership (i.e. the Membership settings).
		...
		mslib3()->net->file_download( wp_json_encode( $data ), $milliseconds . '_membership-settings-export.json' );
	}
	 ...
	 * @since 1.0.0
	 *
	 * @param  MS_Model_Member $src The member to export.
	 *
	 * @return object Export data
	 */
	...
	protected function export_settings() {
		...
		$obj                          = new stdClass();
		$obj->global_payment_settings = $this->get_global_payment_settings( $settings );

		/**
		 * Filters the export settings.
		 *
		 * @since 1.0.0
		 *
		 * @param object            $obj              Export data.
		 * @param MS_Model_settings $current_settings The current settings.
		 *
		 * @return object Export data.
		 */
		return apply_filters(
			'ms_export/export_settings', // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores -- legacy hook.
			$obj,
			$settings
		);
	}
	...
	/**
	 * Returns the global payment settings.
	 *
	 * @since 1.0.3
	 *
	 * @param MS_Model_Settings $settings The current settings.
	 *
	 * @return stdClass
	 */
	protected function get_global_payment_settings( MS_Model_Settings $settings ): stdClass {
		$obj = new stdClass();

		$obj->currency               = $settings->get_currency();
		$obj->invoice_sender_name    = $settings->get_invoice_sender_name();
		$obj->billing_address        = $settings->get_billing_address();
		$obj->company_name           = $settings->get_company_name();
		$obj->company_vax_tax_number = $settings->get_company_vax_tax_number();
