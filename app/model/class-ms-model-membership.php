 ...
 * @property string $trial_period_type
 * @property string $pay_cycle_period_unit
 * @property string $pay_cycle_period_type
 */
class MS_Model_Membership extends MS_Model_CustomPostType {
	...
	/**
	 * Membership non-selected payment type.
	 *
	 * @since 1.0.2
	 * @see $payment_type $payment_type property.
	 */
	 ...
	 * Default is PAYMENT_TYPE_NONE.
	 *
	 * @since 1.0.0
	 * @since 1.0.2 Changed to PAYMENT_TYPE_NONE. See MEMDASH-104.
	 * @var string $payment_type
	 */
	protected $payment_type = self::PAYMENT_TYPE_NONE;
	 ...
	 * Users can still subscribe to the membership via the shortcode
	 ...
	 */
	protected $pay_cycle_period = array(
		'period_unit' => 1,
		'period_type' => 'days',
	);
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label'               => __( 'Membership Memberships', 'memberdash' ),
			'description'         => __( 'Memberships user can join to.', 'memberdash' ),
			'show_ui'             => false,
			'show_in_menu'        => false,
	...
	public static function get_types() {
		$types = array(
			self::TYPE_STANDARD => __( 'Standard Membership', 'memberdash' ),
			self::TYPE_DRIPPED  => __( 'Dripped Content Membership', 'memberdash' ),
			self::TYPE_GUEST    => __( 'Guest Membership', 'memberdash' ),
			self::TYPE_USER     => __( 'Default Membership', 'memberdash' ),
			self::TYPE_BASE     => __( 'System Membership', 'memberdash' ),
		);
	...
	public static function get_payment_types( $type = 'paid' ) {
		if ( 'free' == $type ) {
			$payment_types = array(
				self::PAYMENT_TYPE_NONE       => __( 'Please select access structure', 'memberdash' ),
				self::PAYMENT_TYPE_PERMANENT  => __( 'Permanent access', 'memberdash' ),
				self::PAYMENT_TYPE_FINITE     => __( 'Finite access', 'memberdash' ),
				self::PAYMENT_TYPE_DATE_RANGE => __( 'Date range access', 'memberdash' ),
			);
		} else {
			$payment_types = array(
				self::PAYMENT_TYPE_NONE       => __( 'Please select a payment type', 'memberdash' ),
				self::PAYMENT_TYPE_RECURRING  => __( 'Recurring payments', 'memberdash' ),
				self::PAYMENT_TYPE_PERMANENT  => __( 'One payment for permanent access', 'memberdash' ),
				self::PAYMENT_TYPE_FINITE     => __( 'One payment for finite access', 'memberdash' ),
				self::PAYMENT_TYPE_DATE_RANGE => __( 'One payment for date range access', 'memberdash' ),
			);
		}
	 ...
	 */
	public static function get_membership_ids( $args = null ) {
	 ...
	 * @param  $args The query post args
	 *         @see @link http://codex.wordpress.org/Class_Reference/WP_Query
	 * @param  bool                     $id_as_key Return array indexed by the membership ids.
	 * @return MS_Model_Membership[] The selected memberships.
	 */
	public static function get_memberships( $args = null, $id_as_key = false ) {
		$ids         = self::get_membership_ids( $args );
		$memberships = array();

		$count = 0;
		foreach ( $ids as $id ) {
			$memberships[ $id_as_key ? $id : $count ] = MS_Factory::load(
				'MS_Model_Membership',
				$id
			);
			$count++;
	 ...
	 * @return MS_Model_Membership[] The selected memberships.
	 */
	public static function get_public_memberships() {
		...
		foreach ( $memberships as $key => $item ) {
			if ( ! $item->active ) {
				unset( $memberships[ $key ] ); } elseif ( $item->is_system() ) {
	 ...
	 * @return MS_Model_Membership[] The selected memberships.
	 */
	public static function get_dripped_memberships( $args = null ) {
		$drip_args = array(
			'meta_query' => array(
				array(
					'key'   => 'type',
	...
	public static function get_signup_membership_list(
		...
		if ( $only_names ) {
			...
			if ( ! empty( $private ) ) {
				$priv_key              = __( 'Private Memberships', 'memberdash' );
	 ...
	 * @since 1.0.0
	 *
	 * @param  MS_Model_Membership $a The first membership to compare.
	 * @param  MS_Model_Membership $b The second membership to compare.
	 *
	 ...
	 * @return MS_Model_Membership The Membership.
	 */
	public static function _get_system_membership( $type, $create_missing = true ) {
		...
		if ( ! isset( $Special_Membership[ $type ] ) ) {
			...
			if ( $results ) {
				...
				* In some cases new Membership memberships were created when a
			...
			if ( ! empty( $base ) ) {
				...
				$description             = __( 'Membership Core Membership', 'memberdash' );
	...
	/**
	 * Get Membership base membership.
	 *
	 * Create a new membership if membership does not exist.
	...
	public function __sleep() {
		/*
		 * Rule values are pre-processed before saving...
	...
	public function after_save() {
		// It is important! The Membership membership must be public
	...
	/**
	 * Merge current rules to Membership.
	 *
	 * Assure the membership rules get updated whenever Membership is changed.
	 *
	 * @since 1.0.0
	 * @internal
	 */
	public function prepare_obj() {
		...
		foreach ( $this->rule_values as $key => $values ) {
			...
			// Network-wide: Only instantiate rules for the *current* site!
		...
		// validate rules using Membership rules
	...
	public function get_payment_type_desc() {
		...
		switch ( $this->payment_type ) {
			case self::PAYMENT_TYPE_FINITE:
				if ( $has_payment ) {
					$desc = sprintf(
						__( 'Pay for %1$s', 'memberdash' ),
						MS_Helper_Period::get_period_desc( $this->period, true )
					);
				} else {
					$desc = sprintf(
						__( 'Free for %1$s', 'memberdash' ),
						MS_Helper_Period::get_period_desc( $this->period, true )
					);
				}
				break;
			...
			case self::PAYMENT_TYPE_DATE_RANGE:
				if ( $has_payment ) {
					$desc = sprintf(
						__( 'Pay from %1$s to %2$s', 'memberdash' ),
						$this->period_date_start,
						$this->period_date_end
					);
				} else {
					$desc = sprintf(
						__( 'Free from %1$s to %2$s', 'memberdash' ),
						$this->period_date_start,
						$this->period_date_end
					);
				}
			...
			case self::PAYMENT_TYPE_RECURRING:
				if ( $has_payment ) {
					$desc = __( 'Pay each %1$s', 'memberdash' );
					if ( 1 == $this->pay_cycle_repetitions ) {
						$desc = __( 'Single payment', 'memberdash' );
					} elseif ( $this->pay_cycle_repetitions > 1 ) {
						$desc .= ', ' . __( '%2$s payments', 'memberdash' );
					}
				} else {
					$desc = __( 'Free access', 'memberdash' );
				}
	 ...
	 * ... it is explicitly marked as "free"
	 * ... the price is 0.00
	...
	/**
	 * Returns true if this membership is eligible for trial period.
	 *
	 * @since 1.0.0
	 *
	 * @return bool
	...
	public function get_name_tag( $with_tooltip = false ) {
		if ( $with_tooltip ) {
			$tooltip = sprintf(
				__( 'Revision: %1$s | Type: %2$s', 'memberdash' ),
				$this->revision,
				$this->type
			);
			$tag     = sprintf(
	...
	public function name_tag() {
		echo $this->get_name_tag(); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	...
	/**
	 * Merge Membership rules.
	 *
	 * Merge every rule model with Membership/visitor membership rules.
	 * This ensure rules are consistent with Membership rules.
	 *
	 * @since 1.0.0
	 * @internal
	 */
	...
	public function get_after_ms_ends_options() {
		$options = array(
			0 => __( 'Restrict access to Visitor-Level', 'memberdash' ),
		);
		...
		$label = __( 'Change to: %s', 'memberdash' );
	 ...
	 * @api
	 *
	 * @param int $post_id Optional. The post ID to check access for.
	 *
	...
	public function has_access_to_current_page( $post_id = null ) {
		...
		if ( $this->active ) {
			...
			foreach ( $rules as $rule ) {
				...
				$this->_access_reason[] = sprintf(
					__( '%1$s: Rule "%2$s"', 'memberdash' ),
					$rule_access ? __( 'Allow', 'memberdash' ) : __( 'Deny', 'memberdash' ),
					$rule->rule_type
				);
				...
				// Special pages have final decision after URL groups.
	...
	/**
	 * Returns whether the membership supports trial period.
	 *
	 * @since 1.0.3
	 *
	 * @return bool
	...
	public function supports_trial(): bool {
		// currently only recurring subscriptions support a trial period.
		return $this->supports_recurring_payments();
	}


	/**
	 * Returns whether the membership supports recurring payments.
	 *
	 * @since 1.0.3
	 *
	 * @return bool
	 */
	public function supports_recurring_payments(): bool {
		return $this->payment_type === self::PAYMENT_TYPE_RECURRING;
	}

	/**
	 * Returns the payment type of the membership.
	 *
	 * @since 1.0.3
	 *
	 * @return string
	...
	/**
	 * Returns the priority of the membership.
	 *
	 * @since 1.0.3
	 *
	 * @return int
	...
	public function __get( $property ) {
		...
		switch ( $property ) {
			...
			case 'trial_period_enabled':
				$value = $this->supports_trial() ? $this->trial_period_enabled : false;
				break;
		...
		if ( property_exists( $this, $property ) ) {
			switch ( $property ) {
				...
				case 'payment_type':
					...
					// Disable trial period if it's not supported.
					if ( ! $this->supports_trial() ) {
						$this->trial_period_enabled = false;
					}
					break;

				case 'trial_period_enabled':
					// set false if it's not supported.
					$this->trial_period_enabled = $this->supports_trial() ? mslib3()->is_true( $value ) : false;
					break;
				...
				case 'price':
				case 'trial_price':
					$this->$property = MS_Helper_Cast::to_float( $value );
					break;
				...
				case 'on_end_membership_id':
					if ( 0 == $value ) {
						$this->$property = 0;
					} elseif ( 0 < MS_Factory::load( 'MS_Model_Membership', $value )->id ) {
						$this->$property = $value;
					}
					break;
