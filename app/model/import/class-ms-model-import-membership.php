<?php
/**
 * Imports data from Membership plugin.
 *
 * @since 1.0.0
 * @package MemberDash
 * @subpackage Model
...
class MS_Model_Import_Membership extends MS_Model_Import {
	 ...
	 * @var bool
	 */
	protected static $is_present = null;
	...
	public static function did_import() {
		...
		/**
		 * Allow users to manually declare that some MWPS subscriptions were
		 * imported from old Membership plugin.
		 *
		 * As a result MWPS will additionally listen to the old M1 IPN URL for
	...
	public function prepare() {
		...
		if ( empty( $this->data ) ) {
			self::_message( 'error', __( 'Hmmm, we could not import the Membership data...', 'memberdash' ) );
			return false;
		}
	...
	protected function prepare_import_struct() {
		...
		$this->data->export_time    = ( new Datetime( 'now', wp_timezone() ) )->format( 'Y-m-d H:i' );
		$this->data->notes          = array(
			__( 'Exported data:', 'memberdash' ),
			__( '- Subscription Plans (without level rules)', 'memberdash' ),
			__( '- Members', 'memberdash' ),
			__( '- Registrations (link between Members and Subscription Plans)', 'memberdash' ),
			__( '- Transactions', 'memberdash' ),
			__( 'Each Subscription-Level is imported as a individual Membership.', 'memberdash' ),
			__( 'Transactions are converted to invoices. Data like tax-rate or applied coupons are not available.', 'memberdash' ),
		);
	 ...
	 * The Membership base membership is not included!
	 *
	 * @since 1.0.0
	 */
