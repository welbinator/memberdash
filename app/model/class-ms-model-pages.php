...
class MS_Model_Pages extends MS_Model_Option {
	...
	public static function get_page_types() {
		...
		if ( empty( $Page_types ) ) {
			$Page_types = array(
				self::MS_PAGE_MEMBERSHIPS       => __( 'Membership List', 'memberdash' ),
				self::MS_PAGE_PROTECTED_CONTENT => __( 'Protected Content', 'memberdash' ),
				self::MS_PAGE_REGISTER          => __( 'Registration', 'memberdash' ),
				self::MS_PAGE_REG_COMPLETE      => __( 'Thank-You Page', 'memberdash' ),
				self::MS_PAGE_ACCOUNT           => __( 'Account', 'memberdash' ),
			);
	...
	public static function get_description( $type ) {
		...
		if ( null === $Description ) {
			$Description = array(
				self::MS_PAGE_MEMBERSHIPS       => __( '(Only for logged-in users) List the public memberships and signup.', 'memberdash' ),
				self::MS_PAGE_PROTECTED_CONTENT => __( 'Displayed when a user cannot access the requested page.', 'memberdash' ),
				self::MS_PAGE_REGISTER          => __( '(Only for guests) List the public memberships and the registration form.', 'memberdash' ),
				self::MS_PAGE_REG_COMPLETE      => __( 'Displayed after the user signed up for a membership.', 'memberdash' ),
				self::MS_PAGE_ACCOUNT           => __( 'Shows details about the current user.', 'memberdash' ),
			);
	 ...
	 */
	public static function get_page_by( $field, $value ) {
		...
		if ( ! isset( $Page_list[ $field ][ $value ] ) ) {
			...
			foreach ( $ms_pages as $type => $page ) {
				switch ( $field ) {
					case 'id':
						$found = ( $value === absint( $page->ID ) );
						break;
					case 'slug':
						$found = ( $value === $page->post_name );
						break;
				}
	 ...
	 * @return int
	 */
	public static function get_page_id( $filter ) {
		$page_id = 0;
	...
	/**
	 * Returns details about the site that contains the Membership pages on
	 ...
	 * @return WP_Post|null
	 */
	public static function current_page( $page_id = false, $page_type = null ) {
		static $Res = array();
		...
		$key = wp_json_encode( $page_id ) . wp_json_encode( $page_type );
	 ...
	 */
	public static function is_membership_page( $page_id = null, $page_type = null ) {
		$ms_page_type = false;

		$page_id = apply_filters(
			'ms_model_pages_memberdash_page_id',
			$page_id,
			$page_type
		);
	 ...
	 */
	public static function redirect_to( $page_type, $args = array() ) {
	 ...
	 */
	public static function get_url_after_login( $filter = true ) {
		if ( isset( $_REQUEST['redirect_to'] ) ) {
			$url     = $_REQUEST['redirect_to'];
			$enforce = true; // This redirection was enforced via REQUEST param.
			...
			$url     = add_query_arg(
				...
				self::get_page_url( self::MS_PAGE_REGISTER )
			);
			$enforce = true; // Again, this redirection was enforced via REQUEST param.
		} else {
	 ...
	 */
	public static function get_page_type( $page_id ) {
		static $Types = array();
	 ...
	 */
	public static function create_menu( $page_type, $update_only = null, $update_type = null ) {
		$res = false;
	 ...
	 */
	public static function drop_menu( $page_type ) {
		$res = true;
		...
		switch ( $type ) {
			case self::MS_PAGE_MEMBERSHIPS:
				$lines[] = sprintf(
					...
					__( 'We have the following subscriptions available for our site. You can renew, cancel or upgrade your subscriptions by using the forms below.', 'memberdash' )
				);
			...
			case self::MS_PAGE_REGISTER:
				$lines[] = sprintf(
					'[' . MS_Helper_Shortcode::SCODE_NOTE . ' type="info"]%1$s[/' . MS_Helper_Shortcode::SCODE_NOTE . ']',
					__( 'We have the following subscriptions available for our site. To join, simply click on the Sign Up button and then complete the registration details.', 'memberdash' )
				);
				$lines[] = '[' . MS_Helper_Shortcode::SCODE_SIGNUP . ']';
				break;
			...
			case self::MS_PAGE_REG_COMPLETE:
				$lines[] = sprintf(
					...
					__( 'Your request to join the membership was successfully received!', 'memberdash' ),
					__( 'The Payment Gateway could take a couple of minutes to process and return the payment status.', 'memberdash' )
				);
	...
	public static function create_default_menu() {
		...
		$menu_id = wp_create_nav_menu( __( 'Default Menu', 'memberdash' ) );
