 ...
 * This object is responsible to initialize the the gateway logic.
 *
 * @since 1.0.0
 */
class MS_Model_Gateway extends MS_Model_Option {
	 ...
	 *
	 * @since 1.0.0
	 * @since 1.0.2 Added the $exclude_free parameter.
	 *
	 * @param bool $only_active  Optional. When to return only activated gateways.
	 * @param bool $exclude_free Optional. Use to filter the free gateway from the gateways list.
	 */
	public static function get_gateways( $only_active = false, bool $exclude_free = false ) {
		static $Done = false;
		$res         = null;
	 ...
	 * @since 1.0.0
	 *
	 * @param  int|string $gateway_id The gateway ID.
	 *
	 * @return bool True if the gateway is active.
	 */
	public static function is_active( $gateway_id ) {
		$result          = false;
	 ...
	 * @since 1.0.0
	 */
	protected static function load_core_gateways() {
		...
		// Loop all recognized Gateways and initialize them.
	...
	public static function get_gateway_names( $only_active = false, $include_gateway_free = false ) {
		...
		if ( ! $include_gateway_free ) {
			...
			$names['admin'] = __( 'None (Admin)', 'memberdash' );
		}
