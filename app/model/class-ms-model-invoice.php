...
class MS_Model_Invoice extends MS_Model_CustomPostType {
	 ...
	 * Invoice date.
	 *
	 * This is the date when the INVOICE WAS CREATED. It may be different than the
	 ...
	 * This information is used when price-options of the membership is changed.
	 *
	 * @since 1.0.0
	 * @var int
	 */
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label'               => __( 'Membership Invoices', 'memberdash' ),
			'description'         => __( 'Member Invoices', 'memberdash' ),
			'public'              => false,
			'show_ui'             => false,
			'show_in_menu'        => false,
			'has_archive'         => false,
	...
	public static function get_status_types( $extended = false ) {
		if ( $extended ) {
			$result = array(
				self::STATUS_NEW     => __( 'Draft - Invoice is prepared but user cannot see it yet', 'memberdash' ),
				self::STATUS_BILLED  => __( 'Billed - User can see the invoice and needs to pay', 'memberdash' ),
				self::STATUS_PENDING => __( 'Pending - Waiting for confirmation from payment gateway', 'memberdash' ),
				self::STATUS_PAID    => __( 'Paid - Payment arrived on our account!', 'memberdash' ),
				self::STATUS_DENIED  => __( 'Denied - Payment was denied', 'memberdash' ),
			);
		} else {
			$result = array(
				self::STATUS_NEW     => __( 'Draft', 'memberdash' ),
				self::STATUS_BILLED  => __( 'Billed', 'memberdash' ),
				self::STATUS_PENDING => __( 'Pending', 'memberdash' ),
				self::STATUS_PAID    => __( 'Paid', 'memberdash' ),
				self::STATUS_DENIED  => __( 'Denied', 'memberdash' ),
			);
		}
	 ...
	 * @param int    $invoice_number  The invoice number. The current number if null.
	 * @param string $status          The invoice status. Optional.
	 *
	...
	/**
	 * Returns the current invoice number of a subscription.
	 *
	 * @since 1.0.0
	 *
	...
	public static function get_current_invoice_number( $subscription ) {
		...
		return empty( $current_invoice ) ? 1 : $current_invoice->invoice_number;
	}

	/**
	 * Get current member membership invoice.
	 *
	 * The current invoice is the not paid one. Every time a invoice is paid, the current invoice number is incremented.
	 *
	 * @since 1.0.0
	 *
	 * @param  MS_Model_Relationship $subscription   The membership relationship.
	 * @param  bool                  $create_missing Optional. True to overwrite existing invoice or false to create a new one if doesn't exist.
	 ...
	 * @param bool                  $paid           Is invoice paid already? Otherwise don't set status of the membership as active.
	 *
	 * @throws Exception If the membership is invalid.
	 *
	 * @return MS_Model_Invoice
	 */
	public static function create_invoice( $subscription, $invoice_number = false, $paid = true ) {
		...
		$invoice->name               = apply_filters(
			...
			sprintf(
				// Translators: 1. Membership name, 2. Member username.
				__( 'Invoice for %1$s - %2$s', 'memberdash' ),
				$membership->name,
				$member->get_member_username()
			)
		);
		...
		$invoice->refresh_amount( true ); // it's a new invoice, so force refresh.
	...
	public function archive() {
		if ( $this->id ) {
			...
			$this->add_notes(
				sprintf(
					__( 'Archived on: %s', 'memberdash' ),
					MS_Helper_Period::current_date()
				)
			);
			$this->add_notes(
				sprintf(
					__( 'Former status: %s', 'memberdash' ),
					$this->status
				)
			);
	...
	public function pay_it( $gateway_id = null, $external_id = null ) {
		...
		// Manual gateway works differently. This condition avoids infinite loop.
		...
		// Safety check in case other membership is not removed.
	...
	/**
	 * Makes sure that the pay_date attribute has a valid value.
	 *
	 * @since 1.0.0
	 */
	 ...
	 * Change status accordingly to transaction status.
	 *
	 * @since 1.0.0
	 *
	 ...
	 * @since 1.0.0
	 *
	 * @param string|array $notes The notes to add.
	 *
	 * @return void
	 */
	 ...
	 *
	 * This method is called when we get the invoice amount to update the price, according to some rules.
	 *
	 ...
	 *
	 * @since 1.0.0
	 * @since 1.0.3 Added the $force_price_refresh parameter.
	 *
	 * @param bool $force True to force the price refresh. Default is false.
	 *
	 * @return void
	 */
	private function refresh_amount( bool $force = false ): void {
		...
		// Skip price refresh on no force mode if the timeout is not reached yet or the invoice is not a draft.

		if (
			! $force
			&& $expire_timestamp > time()
			&& self::STATUS_NEW !== $this->status
		) {
			return;
		}

		// Store the current timestamp, so we don't refresh the price until the timeout expires again.
		...
		if ( $subscription->is_trial_eligible() ) {
			$this->trial_price = $membership->trial_price; // Without taxes!
			$this->uses_trial  = true;
		} else {
			$this->trial_price = 0;
			$this->uses_trial  = false;
		}
	...
	public function set_due_date() {
		...
		if ( empty( $due_date ) ) {
			if ( $subscription->is_trial_eligible() ) {
				 ...
				 */
				$due_date = $subscription->calc_trial_expire_date();
			} else {
		...
		$this->trial_ends = $subscription->calc_trial_expire_date();
	 ...
	 *
	 * @since 1.0.0
	 * @since 1.0.3 Added the $refresh_amount parameter.
	 *
	 * @param bool $refresh_amount True to refresh the amount before returning it. Default is true.
	 *
	 * @return float
	 */
	private function get_net_amount( bool $refresh_amount = true ): float {
		if (
			! $this->is_paid()
			&& $refresh_amount
		) {
	 ...
	 * @return float
	 */
	private function get_tax( bool $refresh_amount = true ): float {
		...
		$value = $this->get_net_amount( $refresh_amount ) * ( $tax_rate / 100 );
		if ( $value < 0 ) {
			$value = 0;
		}
	...
	/**
	 * Returns the current total amount of the invoice (including taxes)
	 *
	 * @since 1.0.3
	 *
	 * @return float
	 */
	public function get_calculated_total(): float {
		$total = $this->get_net_amount( false ) + $this->get_tax( false ); // Net amount + tax, without price refresh.
	...
	/**
	 * Returns the external ID of the invoice.
	 *
	 * @since 1.0.3
	 *
	 * @return string
