...
class MS_Model_Communication extends MS_Model_CustomPostType {
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label'               => __( 'Membership Email Templates', 'memberdash' ),
	...
	public static function get_communication_type_titles( $membership = null ) {
		$type_titles = array(
			self::COMM_TYPE_SIGNUP                => __( 'Signup - User account created', 'memberdash' ),
			self::COMM_TYPE_RESETPASSWORD         => __( 'Signup - Forgot Password', 'memberdash' ),
			self::COMM_TYPE_REGISTRATION          => __( 'Subscription - Completed with payment', 'memberdash' ),
			self::COMM_TYPE_REGISTRATION_FREE     => __( 'Subscription - Completed (free membership)', 'memberdash' ),
			self::COMM_TYPE_REGISTRATION_VERIFY   => __( 'Signup - Verify your email', 'memberdash' ),
			self::COMM_TYPE_RENEWED               => __( 'Subscription - Renewed', 'memberdash' ),
			self::COMM_TYPE_BEFORE_FINISHES       => __( 'Subscription - Before expires', 'memberdash' ),
			self::COMM_TYPE_FINISHED              => __( 'Subscription - Expired', 'memberdash' ),
			self::COMM_TYPE_AFTER_FINISHES        => __( 'Subscription - After expired', 'memberdash' ),
			self::COMM_TYPE_CANCELLED             => __( 'Subscription - Cancelled', 'memberdash' ),
			self::COMM_TYPE_BEFORE_TRIAL_FINISHES => __( 'Subscription - Trial finished', 'memberdash' ),
			self::COMM_TYPE_INFO_UPDATE           => __( 'Payment - Profile updated', 'memberdash' ),
			self::COMM_TYPE_CREDIT_CARD_EXPIRE    => __( 'Payment - Credit Card expires', 'memberdash' ),
			self::COMM_TYPE_INVOICE               => __( 'Payment - Receipt/Invoice', 'memberdash' ),
			self::COMM_TYPE_FAILED_PAYMENT        => __( 'Payment - Failed', 'memberdash' ),
			self::COMM_TYPE_BEFORE_PAYMENT_DUE    => __( 'Payment - Before due', 'memberdash' ),
			self::COMM_TYPE_AFTER_PAYMENT_DUE     => __( 'Payment - After due', 'memberdash' ),
		);
	 ...
	 *
	 * @since 1.0.0
	 * @param  int $membership Identifies a membership.
	...
	public function __construct() {
		$this->comm_vars = array(
			self::COMM_VAR_MS_NAME                 => __( 'Subscription: Membership Name', 'memberdash' ),
			self::COMM_VAR_MS_DESCRIPTION          => __( 'Subscription: Membership Description', 'memberdash' ),
			self::COMM_VAR_MS_REMAINING_DAYS       => __( 'Subscription: Remaining days', 'memberdash' ),
			self::COMM_VAR_MS_REMAINING_TRIAL_DAYS => __( 'Subscription: Remaining trial days', 'memberdash' ),
			self::COMM_VAR_MS_EXPIRY_DATE          => __( 'Subscription: Expiration date', 'memberdash' ),
			self::COMM_VAR_MS_INVOICE              => __( 'Subscription: Current Invoice', 'memberdash' ),
			self::COMM_VAR_USER_DISPLAY_NAME       => __( 'User: Display name', 'memberdash' ),
			self::COMM_VAR_USER_FIRST_NAME         => __( 'User: First name', 'memberdash' ),
			self::COMM_VAR_USER_LAST_NAME          => __( 'User: Last name', 'memberdash' ),
			self::COMM_VAR_USERNAME                => __( 'User: Login name', 'memberdash' ),
			self::COMM_VAR_PASSWORD                => __( 'User: Password', 'memberdash' ),
			self::COMM_VAR_RESETURL                => __( 'User: Reset Password URL', 'memberdash' ),
			self::COMM_VAR_MS_ACCOUNT_PAGE_URL     => __( 'Site: User Account URL', 'memberdash' ),
			self::COMM_VAR_BLOG_NAME               => __( 'Site: Name', 'memberdash' ),
			self::COMM_VAR_BLOG_URL                => __( 'Site: URL', 'memberdash' ),
			self::COMM_VAR_VERIFICATION_URL        => __( 'User: Account Verification URL', 'memberdash' ),
		);
		...
		if ( is_multisite() ) {
			$this->comm_vars[ self::COMM_VAR_NET_NAME ] = __( 'Network: Name', 'memberdash' );
			$this->comm_vars[ self::COMM_VAR_NET_URL ]  = __( 'Network: URL', 'memberdash' );
		}
	}
	...
	public function get_description() {
		$description = __( 'Override this description in child class', 'memberdash' );
	...
	public function set_period_name( $field ) {
		$field['title'] = __( 'Period before/after', 'memberdash' );
	 ...
	 * Send email message.
	 *
	 * THIS IS THE ONLY PLACE IN THE MWPS PLUGIN THAT WILL ACTUALLY SEND OUT AN
