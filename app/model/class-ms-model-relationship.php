...
class MS_Model_Relationship extends MS_Model_CustomPostType {
	 ...
	 * @since 1.0.0
	 *
	 * @var bool $trial_period_completed
	 */
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label'               => __( 'Membership Subscriptions', 'memberdash' ),
	...
	public static function get_status_types() {
		$status_types = array(
			self::STATUS_PENDING       => __( 'Pending', 'memberdash' ),
			self::STATUS_ACTIVE        => __( 'Active', 'memberdash' ),
			self::STATUS_TRIAL         => __( 'Trial', 'memberdash' ),
			self::STATUS_TRIAL_EXPIRED => __( 'Trial Expired', 'memberdash' ),
			self::STATUS_EXPIRED       => __( 'Expired', 'memberdash' ),
			self::STATUS_DEACTIVATED   => __( 'Deactivated', 'memberdash' ),
			self::STATUS_CANCELED      => __( 'Canceled', 'memberdash' ),
			self::STATUS_WAITING       => __( 'Not yet active', 'memberdash' ),
		);
	 ...
	 * @param string $move_from_id  A list of membership IDs to cancel on payment. This property is handled by the MS_Model_Invoice class.
	 * @param bool   $paid          Is invoice paid already? Otherwise don't set status of the membership as active.
	 *
	...
	private static function _create_ms_relationship( $membership_id, $user_id, $gateway_id, $move_from_id, $paid = true ) {
		...
		// If status is not pending, set the start/expire dates.

		if ( $subscription->status !== self::STATUS_PENDING ) {
			$subscription->config_period();
		}

		$membership                 = $subscription->get_membership();
		$subscription->payment_type = $membership->get_payment_type();
	 ...
	 * @since 1.0.0
	 *
	 * @param array<mixed> $args The query post args.
	 *         @see @link http://codex.wordpress.org/Class_Reference/WP_Query
	 *
	 * @return array<int> A list of subscription IDs.
	 */
	 ...
	 * Memberships with equal priority are sorted alphabetically.
	 *
	 * @since 1.0.0
	 *
	 * @param MS_Model_Relationship $a The first relationship.
	 * @param MS_Model_Relationship $b The second relationship.
	 *
	 * @return int -1: a < b | 0: a = b | +1: a > b
	 */
	public static function sort_by_priority( $a, $b ) {
		$m1   = $a->get_membership();
		$mwps = $b->get_membership();

		if ( $m1->priority == $mwps->priority ) {
			/**
			...
			return $m1->priority - $mwps->priority;
		}
	}
	...
	/**
	 * Retrieve the membership relationship.
	 *
	 * @since 1.0.0
	 *
	 ...
	 *
	 * @return MS_Model_Relationship The membership relationship.
	 */
	...
	public function cancel_membership( $generate_event = true ) {
		...
		if ( self::STATUS_DEACTIVATED === $this->status ) {
			return;
		}
	...
	public function is_trial_eligible() {
		...
		if ( ! $membership->has_trial() ) {
			...
			$eligible = true;
		} elseif ( ! in_array( $this->status, $trial_eligible_status, true ) && ! $force ) {
	...
	public function set_trial_expire_date( $trial_expire_date = null ) {
		...
		if (
			...
			&& in_array( $this->status, $no_expire_date, true )
		) {
	...
	public function set_expire_date( $expire_date = null ) {
		...
		if ( ! in_array( $this->status, $no_expire_date, true ) ) {
			$valid_date = MS_Helper_Period::is_after(
				$expire_date,
				$this->start_date,
				$this->trial_expire_date
	...
	public function calc_expire_date( $start_date = null, $paid = false ) {
		...
		if ( $this->is_trial_eligible() ) {
			...
			/*
			 * The gateway calls the payment handler URL automatically:
	...
	public function get_membership() {
		if ( empty( $this->membership->id ) ) {
			$this->membership = MS_Factory::load(
				'MS_Model_Membership',
				$this->membership_id,
				MS_Helper_Cast::to_string( $this->id )
			);
	...
	public function get_payment_description( $invoice = null, $short = false ) {
		...
		if ( null !== $invoice ) {
			$total_price = $invoice->get_calculated_total(); // Includes Tax.
			...
			$trial_due   = $this->trial_expire_date; // Trial end.
			$trial_ends  = $this->trial_expire_date;
		}
		...
		switch ( $payment_type ) {
			case MS_Model_Membership::PAYMENT_TYPE_PERMANENT:
				if ( 0 == $total_price ) {
					if ( $short ) {
						$lbl = __( 'Nothing (for ever)', 'memberdash' );
					} else {
						$lbl = __( 'You will pay nothing for permanent access.', 'memberdash' );
					}
				} else {
					if ( $short ) {
						$lbl = __( '<span class="price">%1$s %2$s</span> (for ever)', 'memberdash' );
					} else {
						$lbl = __( 'You will pay %1$s %2$s for permanent access.', 'memberdash' );
					}
				}

				if ( MS_Model_Member::is_admin_user() ) {
					$desc = __( 'Admin has no fees!', 'memberdash' );
				} else {
					$desc = sprintf(
						$lbl,
						$currency,
			...
			case MS_Model_Membership::PAYMENT_TYPE_FINITE:
				if ( 0 == $total_price ) {
					if ( $short ) {
						$lbl = __( 'Nothing (until %4$s)', 'memberdash' );
					} else {
						$lbl = __( 'You will pay nothing for access until %3$s.', 'memberdash' );
					}
				} else {
					if ( $short ) {
						$lbl = __( '<span class="price">%1$s %2$s</span> (until %4$s)', 'memberdash' );
					} else {
						$lbl = __( 'You will pay %1$s %2$s for access until %3$s.', 'memberdash' );
					}
				}
			...
			case MS_Model_Membership::PAYMENT_TYPE_DATE_RANGE:
				if ( 0 == $total_price ) {
					if ( $short ) {
						$lbl = __( 'Nothing (%5$s - %6$s)', 'memberdash' );
					} else {
						$lbl = __( 'You will pay nothing for access from %3$s until %4$s.', 'memberdash' );
					}
				} else {
					if ( $short ) {
						$lbl = __( '<span class="price">%1$s %2$s</span> (%5$s - %6$s)', 'memberdash' );
					} else {
						$lbl = __( 'You will pay %1$s %2$s to access from %3$s until %4$s.', 'memberdash' );
					}
				}
			...
			case MS_Model_Membership::PAYMENT_TYPE_RECURRING:
				...
				if ( 1 == $membership->pay_cycle_repetitions ) {
					...
					if ( $short ) {
						$lbl = __( '<span class="price">%1$s %2$s</span> (once)', 'memberdash' );
					} else {
						$lbl = __( 'You will pay %1$s %2$s once.', 'memberdash' );
					}
				} else {
					if ( $membership->pay_cycle_repetitions > 1 ) {
						// Fixed number of payments (more than 1)
						if ( $short ) {
							$lbl = _n( 'One time <span class="price">%1$s %2$s</span>.', '%4$s times <span class="price">%1$s %2$s</span> (each %3$s)', $no_payment_rest, 'memberdash' ); //phpcs:ignore WordPress.WP.I18n.MismatchedPlaceholders,WordPress.WP.I18n.MissingSingularPlaceholder
						} else {
							$lbl = _n( 'You will make a payment of %1$s %2$s.', 'You will make %4$s payments of %1$s %2$s, one each %3$s.', $no_payment_rest, 'memberdash' ); //phpcs:ignore WordPress.WP.I18n.MismatchedPlaceholders,WordPress.WP.I18n.MissingSingularPlaceholder
						}
					} else {
						// Indefinite number of payments
						if ( $short ) {
							$lbl = __( '<span class="price">%1$s %2$s</span> (each %3$s)', 'memberdash' );
						} else {
							$lbl = __( 'You will pay %1$s %2$s each %3$s.', 'memberdash' );
						}
					}
				}
		...
		if ( $this->is_trial_eligible() && 0 != $total_price ) {
			if ( empty( $trial_ends ) ) {
				$trial_ends = $this->calc_trial_expire_date();
			}

			if ( empty( $trial_due ) ) {
				$trial_due = $this->calc_trial_expire_date();
			}

			if ( 0 == absint( $trial_price ) ) {
				if ( $short ) {
					if ( MS_Model_Membership::PAYMENT_TYPE_RECURRING == $payment_type ) {
						$lbl = __( 'after %4$s', 'memberdash' );
					} else {
						$lbl = __( 'on %4$s', 'memberdash' );
					}
				} else {
					$trial_price = __( 'nothing', 'memberdash' );
					$lbl         = __( 'Your %1$s free trial ends on %5$s and then you will be billed.', 'memberdash' );
				}
			} else {
				$trial_price = MS_Helper_Billing::format_price( $trial_price );
				$lbl         = __( 'For the trial period of %1$s you only pay %2$s %3$s.', 'memberdash' );
			}
	 ...
	 *         could also be TRIAL, in which case the return value is false.
	 */
	public function add_payment( $amount, $gateway, $external_id = '' ) {
		...
		// If start date is not set, set it now.
		...
		if (
			...
			/*
			 * Important:
			 * FIRST set the TRIAL EXPIRE DATE, otherwise status is set to active instead of trial!
			 */
			$this->set_trial_expire_date();
	...
	/**
	 * Update paid subscription metadata
	 */
	public function update_paid_subscription() {
		$payments = get_post_meta( $this->id, 'payments' );
		if ( empty( $payments ) ) {
			update_post_meta( $this->id, 'payments', $this->payments );
		} else {
			array_merge( $payments, $this->payments );
			update_post_meta( $this->id, 'payments', $payments );
		}
		update_post_meta( $this->id, 'status', $this->status );
		update_post_meta( $this->id, 'trial_expire_date', $this->trial_expire_date );
		update_post_meta( $this->id, 'trial_period_completed', $this->trial_period_completed );
		update_post_meta( $this->id, 'expire_date', $this->expire_date );
	}
	...
	public function set_status( $status ) {
		...
		if ( in_array( $status, $ignored_status, true ) ) {
		...
		/**
		 * Trigger an action to allow other plugins to observe a change in an
		 * subscription status.
		 *
		 * @since 1.0.0
	...
	/**
	 * Get the membership relationship status.
	 *
	 * @since 1.0.0
	 *
	 ...
	 * @param bool   $force_admin Whether to force the admin approach.
	 *
	 * @return void
	 */
	public function set_gateway( $new_gateway, $force_admin = false ) {
		...
		// In case the gateway is admin, we need to set it to the default active gateway.
		...
		if ( $old_gateway && MS_Gateway_Free::ID === $new_gateway ) {
			return;
		}
	 ...
	 * trial expire date and expire date.
	 *
	 * @since 1.0.0
	 *
	...
	protected function calculate_status( $set_status = null, $debug = false ) {
		...
		$set_status = ! empty( $set_status ) ? $set_status : $this->get_status();
		...
		if ( $check_trial ) {
			...
			if ( ! $calc_status
				&& ( ( $force && empty( $this->trial_expire_date ) && $set_status === self::STATUS_TRIAL )
					|| strtotime( $this->trial_expire_date ) >= strtotime(
						MS_Helper_Period::current_date()
					) ) ) {
		...
		if ( 'admin' === $this->gateway_id ) {
			...
			$valid_payment = false;
			// Check if there is *any* payment, no matter what about.
			...
			if ( ! $valid_payment ) {
				...
				for ( $ind = $this->current_invoice_number; $ind > 0; $ind-- ) {
	...
	public function handle_status_change( $new_status ) {
		...
		if ( ! array_key_exists( $new_status, self::get_status_types() ) ) {
			return false;
		}

		if ( $this->is_simulated ) { //phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedIf
	...
	public function get_status_description() {
		...
		switch ( $this->status ) {
			case self::STATUS_PENDING:
				$desc = __( 'Pending payment.', 'memberdash' );
				break;

			case self::STATUS_TRIAL:
				$desc = sprintf(
					'%s <span class="ms-date">%s</span>',
					__( 'Membership Trial expires on ', 'memberdash' ),
			...
			case self::STATUS_ACTIVE:
				if ( ! empty( $this->expire_date ) ) {
					$desc = sprintf(
						'%s <span class="ms-date">%s</span>',
						__( 'Membership expires on ', 'memberdash' ),
						MS_Helper_Period::format_date( $this->expire_date )
					);
				} else {
					$desc = __( 'Permanent access.', 'memberdash' );
				}
				break;
			...
			case self::STATUS_EXPIRED:
				$desc = sprintf(
					'%s <span class="ms-date">%s</span>',
					__( 'Membership expired since ', 'memberdash' ),
					MS_Helper_Period::format_date( $this->expire_date )
				);
				break;
			...
			case self::STATUS_CANCELED:
				$desc = sprintf(
					'%s <span class="ms-date">%s</span>',
					__( 'Membership canceled, valid until it expires on ', 'memberdash' ),
					MS_Helper_Period::format_date( $this->expire_date )
				);
				break;

			case self::STATUS_DEACTIVATED:
				$desc = __( 'Membership deactivated.', 'memberdash' );
				break;
		}
	...
	public function check_membership_status() {
		 ...
		 * Membership from sending *any* emails to users.
		...
		switch ( $next_status ) {
			...
			case self::STATUS_CANCELED:
				...
				if ( $days->remaining <= 0 ) {
					...
					// if there was no another membership configured then this membership deactivate
					if ( $days->remaining <= 0 && $new_membership_id == 0 ) {
						$this->handle_status_change( self::STATUS_DEACTIVATED );
						$this->save();
					}
				}
				...
				if ( $membership->get_payment_type() === MS_Model_Membership::PAYMENT_TYPE_FINITE ) {
	...
	/**
	 * Returns whether the payment is already processed.
	 *
	 * @param string $external_id The external ID of the payment (e.g. Stripe invoice ID).
	 *
	 * @return bool
	 */
	public function is_payment_processed( string $external_id ): bool {
		$last_processed_id = $this->get_current_invoice()->get_external_id();

		if ( ! empty( $last_processed_id ) ) {
			return $last_processed_id === $external_id;
		}

		// Loop through all payments and check if the external ID matches.
