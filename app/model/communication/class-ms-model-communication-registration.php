...
class MS_Model_Communication_Registration extends MS_Model_Communication {
	...
	public function get_description() {
		return __(
			'Sent when a member completes the signup for a paid membership.',
			'memberdash'
		);
	}
	...
	public function reset_to_default() {
		...
		$this->subject = sprintf(
			__( 'Confirmation of your membership at %s', 'memberdash' ),
	...
	public static function get_default_message() {
		...
		$body_notice  = sprintf(
			__( 'Thank you! Your <strong>%1$s</strong> Membership at %2$s is now activated!', 'memberdash' ),
			self::COMM_VAR_MS_NAME,
			self::COMM_VAR_BLOG_NAME
		);
		$body_account = sprintf(
			__( 'You can review and edit your membership details here: %1$s', 'memberdash' ),
			self::COMM_VAR_MS_ACCOUNT_PAGE_URL
		);
		$body_invoice = __( 'Here is the most recent payment information for your subscription:', 'memberdash' );
	...
	public function process_communication( $event, $subscription ) {
		...
		// Email for free memberships is in MS_Model_Communication_Registration_Free.
