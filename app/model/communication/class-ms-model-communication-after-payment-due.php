...
class MS_Model_Communication_After_Payment_Due extends MS_Model_Communication {
	...
	public function set_period_name( $field ) {
		$field['title'] = __( 'Message Delay', 'memberdash' );
		$field['desc']  = __( 'Use "0" to send on due date, or another value to delay the message.', 'memberdash' );
	...
	public function get_description() {
		return __(
			'A payment reminder that is sent a predefined number of days after the payment is due (only if the invoice was not paid until then). You must decide how many days after a message is to be sent.',
			'memberdash'
		);
	}
	...
	public function reset_to_default() {
		...
		$this->subject        = __( 'Membership payment due', 'memberdash' );
	...
	public static function get_default_message() {
		...
		$body_notice  = sprintf(
			__( 'This is a reminder that the payment for your %1$s membership at %2$s is now due (%3$s).', 'memberdash' ),
			self::COMM_VAR_MS_NAME,
			self::COMM_VAR_BLOG_NAME,
			self::COMM_VAR_MS_EXPIRY_DATE
		);
		$body_invoice = __( 'Here are your latest invoice details:', 'memberdash' );
