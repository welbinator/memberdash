...
class MS_Model_Communication_Cancelled extends MS_Model_Communication {
	...
	public function set_period_name( $field ) {
		...
		$field['desc']  = __( 'Use "0" to send instantly, or another value to delay the message.', 'memberdash' );
	...
	public function get_description() {
		return __( 'Sent when membership is cancelled.', 'memberdash' );
	}
	...
	public function reset_to_default() {
		...
		$this->subject        = sprintf(
			__( 'Your %s membership was canceled', 'memberdash' ),
	...
	public static function get_default_message() {
		...
		$body_notice   = sprintf(
			__( 'Your %1$s membership at %2$s was successfully cancelled.', 'memberdash' ),
			self::COMM_VAR_MS_NAME,
			self::COMM_VAR_BLOG_NAME
		);
		$body_register = sprintf(
			__( 'Should you change your mind, you can renew your membership here: %1$s', 'memberdash' ),
			self::COMM_VAR_MS_ACCOUNT_PAGE_URL
		);
		$body_payments = __( 'Here are your latest payment details:', 'memberdash' );
