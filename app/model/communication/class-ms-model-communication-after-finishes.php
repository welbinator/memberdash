...
class MS_Model_Communication_After_Finishes extends MS_Model_Communication {
	...
	public function get_description() {
		return __(
			'Sent a predefined number of days after the membership finishes. You must decide how many days after a message is to be sent.',
			'memberdash'
		);
	}
	...
	public function reset_to_default() {
		...
		$this->subject        = sprintf(
			__( 'Reminder: your %s membership has ended', 'memberdash' ),
	...
	public static function get_default_message() {
		...
		$body_notice = sprintf(
			__( 'This is a reminder that your %1$s membership at %2$s has ended on %3$s.', 'memberdash' ),
			self::COMM_VAR_MS_NAME,
			self::COMM_VAR_BLOG_NAME,
			self::COMM_VAR_MS_EXPIRY_DATE
		);
		$body_renew  = sprintf(
			__( 'You can renew your membership here: %1$s', 'memberdash' ),
