...
class MS_Addon_Coupon_Model extends MS_Model_CustomPostType {
   ...
   *
	...
	/**
	 * Maximum times this coupon could be used.
	 ...
	 *
	 *  The max_uses is 15. Now 15 users visit the payment page and enter the
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label'               => __( 'Membership Coupons', 'memberdash' ),
	 ...
	 *     @type string $discount_symbol The discount symbol.
	 * }
	 */
	...
	public static function get_discount_duration() {
		...
		if ( empty( $durations ) ) {
			$durations = array(
				self::DURATION_ONCE   => __( 'the first invoice', 'memberdash' ),
				self::DURATION_ALWAYS => __( 'to all invoices', 'memberdash' ),
			);
		}
	 ...
	 *
	 * Checks for maximum number of uses, date range and membership_id restriction.
	 *
	 * @since 1.0.0
	 *
	 * @param int              $membership_id The membership id for which coupon is applied.
	 * @param MS_Model_Invoice $invoice       The invoice for which coupon is applied.
	 *
	...
	public function is_valid( $membership_id = 0, $invoice = null ) {
		...
		if ( empty( $this->code ) ) {
			$this->coupon_message = __( 'Coupon code not found.', 'memberdash' );
			$valid                = false;
		} elseif ( $this->max_uses && $this->used >= $this->max_uses ) {
			$this->coupon_message = __( 'No Coupons remaining for this code.', 'memberdash' );
			$valid                = false;
		} elseif ( ! empty( $this->start_date ) && $start_date_timestamp > $timestamp ) {
			$this->coupon_message = __( 'This Coupon is not valid yet.', 'memberdash' );
			$valid                = false;
		} elseif ( ! empty( $this->expire_date ) && $expire_date_timestamp < $timestamp ) {
			$this->coupon_message = __( 'This Coupon has expired.', 'memberdash' );
			$valid                = false;
		} elseif ( self::DURATION_ONCE === $this->duration && ! is_null( $invoice ) ) {
			if ( $invoice->is_valid() && 1 < $invoice->invoice_number ) {
				$this->coupon_message = __( 'This Coupon can be used only in first payment.', 'memberdash' );
				$valid                = false;
			}
		} else {
			...
			if ( ! $membership_allowed ) {
				$this->coupon_message = __( 'This Coupon is not valid for this membership.', 'memberdash' );
				$valid                = false;
			}
		}
	...
	public function get_discount_value( $subscription ) {
		...
		if ( $this->is_valid( $membership->id, $subscription->get_current_invoice( false ) ) ) {
			...
			$this->coupon_message = sprintf(
				__( 'Coupon applied: %1$s', 'memberdash' ),
				$this->code
			);
		}
	...
	/**
	 * Returns the coupon discount value.
	 *
	 * @since 1.0.1
	 *
	 * @return float
