...
class MS_Addon_Wpml extends MS_Addon {
	...
	public function init() {
		...
		if ( self::is_active() ) {
			...
			// Fix stuff in the Membership admin pages.
			$this->add_filter(
	...
	public function register( $list ) {
		$list[ self::ID ] = (object) array(
			...
			'name'        => __( 'WPML Integration', 'memberdash' ),
			'description' => __( 'Use WPML to translate plugin messages.', 'memberdash' ),
		);

		if ( ! self::wpml_active() ) {
			$list[ self::ID ]->description .= sprintf(
				'<br /><b>%s</b>',
				__( 'Activate WPML to use this Add-on', 'memberdash' )
			);
			$list[ self::ID ]->action       = '-';
		}
	...
	protected function check_requirements() {
		if ( ! self::wpml_active() ) {
			mslib3()->ui->admin_message(
				sprintf(
					'<b>%s</b><br>%s',
					__( 'WPML not active!', 'memberdash' ),
					__( 'Heads up: Activate the WPML plugin to enable all translation features of MemberDash.', 'memberdash' )
				),
				'err'
			);
			return;
		...
		if ( version_compare( ICL_SITEPRESS_VERSION, '3.0.0', 'lt' ) ) {
			mslib3()->ui->admin_message(
				sprintf(
					'<b>%s</b><br>%s',
					__( 'Great, you\'re using WPML!', 'memberdash' ),
					__( 'Heads up: Your version of WPML is outdated. Please update WPML to version <b>3.0.0 or higher</b> to enable all translation features of MemberDash.', 'memberdash' )
				),
				'err'
			);
			return;
		...
		if ( ! defined( 'WPML_ST_VERSION' ) ) {
			mslib3()->ui->admin_message(
				sprintf(
					'<b>%s</b><br>%s',
					__( 'Great, you\'re using WPML!', 'memberdash' ),
					__( 'Heads up: To enable all the translation features of MemberDash you need to activate the <b>WPML String Translation</b> module.', 'memberdash' )
				),
				'err'
			);
			return;
				...
				mslib3()->ui->admin_message(
					sprintf(
						...
						// translators: %1$s is the opening <a> tag, %2$s is the closing </a> tag.
							__( 'Heads up: For the best translation experience we recommend to enable "<b>Set admin language as editing language</b>" in your %1$suser profile%2$s.', 'memberdash' ), // cspell:disable-line.
	...
	public function show_flag( $content, $field ) {
		...
		if ( $supported ) {
			$label = __( 'WPML Support', 'memberdash' );
			$info  = __( 'Translate by using the language switch in the toolbar.', 'memberdash' );
		} else {
			$label     = __( 'WPML not supported', 'memberdash' );
			$info      = __( 'Required module missing', 'memberdash' );
			$classes[] = 'warning';
			$icon     .= '<i class="memberdash-fa memberdash-fa-exclamation-circle warning-icon"></i>';
		}
	 ...
	 *
	 * We use this filter to hide the "Membership Pages" section when not in
	 * default-language mode.
	 *
	 * @since 1.0.0
	 *
	 * @param  array            $fields List of HTML field definitions.
	 * @param  MS_View_Settings $view   The Settings view.
	 *
	...
	public function change_general_settings( $fields, $view ) {
		if ( $this->current_lang != $this->default_lang ) {
			$fields['pages'] = sprintf(
				__( 'You can change your settings for the Membership Pages only when you switch to your <b>default language</b> (%s)', 'memberdash' ),
