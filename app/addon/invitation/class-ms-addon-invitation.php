...
class MS_Addon_Invitation extends MS_Addon {
	...
	public function register( $list ) {
		$list[ self::ID ] = (object) array(
			'name'        => __( 'Invitation Codes', 'memberdash' ),
			'description' => __( 'Users need an invitation code to subscribe to a membership.', 'memberdash' ),
		);
	...
	public function menu_item( $items, $limited_mode, $controller ) {
		if ( ! $limited_mode ) {
			$menu_item = array(
				self::ID => array(
					'title' => __( 'Invitation Codes', 'memberdash' ),
					'slug'  => self::SLUG,
				),
			);
	...
	public function membership_option( $view, $membership ) {
		...
		$fields = array(
			...
			array(
				...
				'title'     => __( 'Does this Membership require an Invitation code?', 'memberdash' ),
				'value'     => mslib3()->is_true( $membership->get_custom_data( 'no_invitation' ) ),
				'before'    => sprintf(
					'%s <i class="memberdash-fa memberdash-fa-lock"></i>',
					__( 'Yes', 'memberdash' )
				),
				'after'     => sprintf(
					'<i class="memberdash-fa memberdash-fa-unlock"></i> %s',
					__( 'No (public membership)', 'memberdash' )
				),
				'class'     => 'reverse',
				'ajax_data' => array(
					'action'        => $action,
		...
		if ( ! empty( $data['invitation_valid'] ) ) {
			$fields = array(
				...
				'remove_invitation_code' => array(
					...
					'title'        => sprintf(
						__( 'Using invitation code %s.', 'memberdash' ),
						'<strong>' . $code . '</strong>'
					),
					'button_value' => 1,
				),
			...
			$fields = array(
				...
				'apply_invitation_code' => array(
					...
					'type'  => MS_Helper_Html::INPUT_TYPE_SUBMIT,
					'value' => __( 'Apply Invitation', 'memberdash' ),
				),
			);

			if ( ! $message ) {
				$message = __( 'You need an invitation to register for this Membership', 'memberdash' );
			}
		}
				...
				<form method="post">
					...
					<div class="invitation-entry">
						<?php if ( ! isset( $data['invitation_valid'] ) ) : ?>
							...
							<?php
							esc_html_e( 'Have an invitation code?', 'memberdash' );
							?>
							</div>
							<?php
						endif;
