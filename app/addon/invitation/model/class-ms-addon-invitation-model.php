...
class MS_Addon_Invitation_Model extends MS_Model_CustomPostType {
	...
	/**
	 * Maximum times this invitation could be used.
	 *
	 * @since 1.0.0
	 *
	 * @var int
	...
	public static function load_by_code( $code ) {
		...
		if ( ! $model->is_valid() ) {
			$model->invitation_message = __( 'Invitation code not found.', 'memberdash' );
		}
	...
	public static function get_application( $user_id, $membership_id ) {
		...
		if ( $invitation->is_valid() ) {
			$invitation->invitation_message = __( 'Invitation code is correct.', 'memberdash' );
		}
	 ...
	 *
	 * Checks for maximum number of uses, date range, if the user has used it
	 * and if it exists.
	 *
	 * @since 1.0.0
	 *
	...
	public function is_valid( $membership_id = 0 ) {
		...
		if ( $this->max_uses && $this->used >= $this->max_uses ) {
			$this->invitation_message = __( 'This invitation code is no longer valid.', 'memberdash' );
			$valid                    = false;
		} elseif ( ! empty( $this->start_date ) && strtotime( $this->start_date ) > $timestamp ) {
			$this->invitation_message = __( 'This invitation is not valid yet.', 'memberdash' );
			$valid                    = false;
		} elseif ( ! empty( $this->expire_date ) && strtotime( $this->expire_date ) < $timestamp ) {
			$this->invitation_message = __( 'This invitation has expired.', 'memberdash' );
			$valid                    = false;
			...
			if ( ! $membership_allowed ) {
				$this->invitation_message = __( 'This Invitation is not valid for this membership.', 'memberdash' );
				$valid                    = false;
			} else {
