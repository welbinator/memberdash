...
/**
 * Class-Loader code.
 * Initializes the autoloader and required plugin hooks.
 *
 * @since 1.0.0
 */
class MS_Loader {
	...
	public function ms_class_path_overrides( $overrides ) {
		...
		$core      = array(
			...
			'MS_Licensing'       => 'class-ms-licensing.php',
			'MS_Auth'            => 'class-ms-auth.php',
			'MS_Controller'      => 'class-ms-controller.php',
			'MS_DashboardWidget' => 'class-ms-dashboardwidget.php',
		...
		$models      = array(
			...
			'MS_Model_Api'                                 => 'class-ms-model-api.php',
		);
	...
	private function class_loader( $class ) {
		...
		if ( array_key_exists( $class, $Path_overrides ) ) {
			 ...
			 */
			$file_path = MEMBERDASH_PLUGIN_BASE_DIR . '/' . $Path_overrides[ $class ];
			...
			/**
			 * Case 2: The class-path is not explicitly defined in $Path_overrides.
			...
			foreach ( $paths as $type => $path ) {
				$candidates[] = MEMBERDASH_PLUGIN_BASE_DIR . '/' . $path . '/' . $file_path;
				$candidates[] = MEMBERDASH_PLUGIN_BASE_DIR . '/' . $path . '/' . $file_path_alt;
			}
